...
\begin{equation}
	...
	\quad \text{with effective quark masses} \quad
...
\end{equation}
With vacuum measurements of the two decay constants and three of the meson masses \eqref{eq:lsm:mass_system_3f},
we determine the three parameters $m^2$, $\lambda_1$ and $\lambda_2$ and use them to predict the remaining meson masses.
It is then straightforward to calculate the symmetry breakers $h_x$ and $h_y$ from equation \eqref{eq:lsm3f:symmetry_breakers}.
...
We have continued to use $\sigma$ and $\pi$ masses for consistency with our two-flavor analysis and added the third $K$ mass, as there are the mesons with lowest energy.
...
neglecting pion condensation by keeping $\avg{\pi_a}=0$
...
only now we have a different vacuum potential $\pot(\avg{\sigma},\avg{\pi})$, $N_f=3$ flavors and must remember $m_u = m_d \neq m_s$.
Before removing the vacuum divergence, we then have
\begin{equation}
\begin{split}
	\Omega(\avg{\sigma_x},\avg{\sigma_y},\vec{\mu}) &= \pot(\avg{\sigma},\avg{\pi}) + N_c \smashoperator{\sum_{\vphantom{\big|}f=\{u,d,s\}}} \frac{m_f^4}{16 \pi^2} \left[ \frac{1}{\epsilon} + \frac{3}{2} + \log\left(\frac{{\Lambda}^2}{m_f^2}\right) \right] \\
...
\end{equation}
can be removed by shifting the $\lambda_2$ coupling in the vacuum meson potential \eqref{eq:lsm:potential_tree_3f} like
\begin{equation}
...
\begin{equation}
\begin{split}
	\Omega(\avg{\sigma_x},\avg{\sigma_y},\vec{\mu}) &= \pot(\avg{\sigma},0) + N_c \smashoperator{\sum_{\vphantom{\big|} f=\{u,d,s\}}} \frac{m_f^4}{16 \pi^2} \left[ \frac{3}{2} + \log\left(\frac{{\Lambda}^2}{m_f^2}\right) \right] \\
	                                                &-\smashoperator{\sum_{\vphantom{\big|} f=\{u,d,s\}}} \frac{N_c}{24 \pi^2} \left[ \left( 2 \mu_f^2 - 5 m_q^2 \right) \mu_f \sqrt{\mu_f^2 - m_q^2} + 3 m_q^4 \asinh \left( \sqrt{\frac{\mu_{\smash{f}}^2}{m_q^2}-1} \right) \right] \\
...
\end{align}
\end{subequations}
However, we can only operate with one renormalization parameter $\Lambda$ \TODO{why? why not two separate $\Lambda_x$ and $\Lambda_y$? surely cannot be a big difference?}.
We let all three quark flavors have an equal say by taking the average
\begin{equation}
	\Lambda = \frac{2 \Lambda_x + \Lambda_y}{3} = \SI{208.0}{\mega\electronvolt}.
\label{eq:lsm3f:renormalization_average}
\end{equation}
As the partial of the second term of the second term on the first line of the grand potential \eqref{eq:lsm3f:grand_potential} no longer vanishes at the original minimum \eqref{eq:lsm3f:decay_constants},
\emph{its minimum changes with this procedure}.
A good first thought would be to look for the new minimum as the point where the partial derivative of this term vanishes,
but at that point the partial derivative of $\pot(\avg{\sigma},0)$ would no longer vanish!
Consequently, \emph{the new minimum generally changes with any parameter of the model!}.

Whether it is most accurate to operate with $\Lambda_x$ and $\Lambda_y$ or only $\Lambda$ depends on the physical interpretation of the renormalization scales.
Interesting to note that $\Lambda$ is closest to QCD Landau pole around $\SI{200}{\mega\electronvolt}$, \cite{ref:presentation_renormalization_scale}
where one coupling in quantum chromodynamics explodes and perturbation theory breaks down.
If we can interpret $\Lambda$ as a cutoff beyond which , then the value \eqref{eq:lsm3f:renormalization_average} is indeed the one that is most comparable to the Landau pole around $\SI{200}{\mega\electronvolt}$ of quantum chromodynamics.
\TODO{???}
\TODO{someone that uses multiple renormalization scales: \url{https://journals.aps.org/prd/abstract/10.1103/PhysRevD.55.2202}}

\emph{This shifts the minimum of the grand potential \eqref{eq:lsm3f:grand_potential} in vacuum away from \eqref{eq:lsm:mass_system_3f},
and the minimum will generally change with any parameter in the model!}
...
\end{axis}
\end{tikzpicture}
\caption{\TODO{caption, add 600-700-800}
