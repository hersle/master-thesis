...
\TODO{what more to write in intro?}

\TODO{demonstrate generalization from 2 to 3 flavors?}

\TODO{rewrite, structure everything with subheaders}

\TODO{short comment on using different normalization convention for potential couplings, because they are fit to experimental values anyway}
...
\TODO{reflect the confinement thing in LSM2F chapter}
...
\TODO{relate to renormalization? significant?}
...
\TODO{why? what does it mean when people write that ``$\avg{\phi} \neq 0$ must carry the same quantum numbers as the vacuum, so therefore only its diagonal part can be nonzero''???}
...
\TODO{why? want $u$ and $d$ treated equally, and $s$ differently. look at Gell-Mann matrices, see that only $0$ and $8$ does such a thing.  want to break $SU(3)$, this is why i choose the non-zero $h$. but $c=0$ is a choice -- no anomaly},

\TODO{later consider effects of anomaly coupling $c$?}
...
Using the program in \TODO{create and ref short appendix},
...
offset by constants to make them comparable to the human eye.
}
\end{figure}
...
\nextgroupplot[
	...
	ymin=-20, ymax=1100, ytick distance=100, 
	%ymax=600, 
	title={\subcaption{\label{fig:lsm:3-flavor-eos-parametrization}Parametrization of solutions}},
	legend cell align=left, legend pos=north west, legend columns=6, legend transposed, reverse legend,
];
...
\addplot+ {-100}; \addlegendimage{empty legend} \addlegendentry{}
\addplot+ {-100}; \addlegendimage{empty legend} \addlegendentry{}
\addplot+ {-100}; \addlegendimage{empty legend} \addlegendentry{}
...
\addplot+ [blue,      solid,          opacity=0.7,            ] table [x expr={(\thisrow{muu}+\thisrow{mud})/2}, y=mue] {../code/data/LSM3F/eos_sigma_800.dat}; \addlegendentry{$\mu_e \, / \, \si{\mega\electronvolt}$};
...
\addplot+ [darkgreen, solid,          opacity=0.7,            ] table [x expr={(\thisrow{muu}+\thisrow{mud})/2}, y=mus] {../code/data/LSM3F/eos_sigma_800.dat}; \addlegendentry{$\mu_s \, / \, \si{\mega\electronvolt}$};
...
\addplot+ [darkgreen, solid,          opacity=0.7,            ] table [x expr={(\thisrow{muu}+\thisrow{mud})/2}, y=mud] {../code/data/LSM3F/eos_sigma_800.dat}; \addlegendentry{$\mu_d \, / \, \si{\mega\electronvolt}$};
...
\addplot+ [red,       solid,          opacity=0.7,            ] table [x expr={(\thisrow{muu}+\thisrow{mud})/2}, y=muu] {../code/data/LSM3F/eos_sigma_800.dat}; \addlegendentry{$\mu_u \, / \, \si{\mega\electronvolt}$};
...
\addplot+ [yellow,    solid,          opacity=0.7,            ] table [x expr={(\thisrow{muu}+\thisrow{mud})/2}, y=ms]  {../code/data/LSM3F/eos_sigma_800.dat}; \addlegendentry{$m_y \, / \, \si{\mega\electronvolt}$};
...
\addplot+ [orange,    solid,          opacity=0.7,            ] table [x expr={(\thisrow{muu}+\thisrow{mud})/2}, y=mu]  {../code/data/LSM3F/eos_sigma_800.dat}; \addlegendentry{$m_x \, / \, \si{\mega\electronvolt}$};
...
\addplot+ [black, dotted] {-1}; \addlegendentry{$m_f=m_e=0$};
...
\addplot+ [gray, densely dashed, opacity=0.7] table [x=Porg,y=epsilonorg] {../code/data/LSM3F/eos_sigma_700.dat}; \addlegendentry{$m_\sigma=\SI{700}{\mega\electronvolt}$ (uncorrected)};
\addplot+ [black, densely dashed, opacity=0.7] table [x=P,y=epsilon] {../code/data/LSM3F/eos_sigma_700.dat}; \addlegendentry{$m_\sigma=\SI{700}{\mega\electronvolt}$ (corrected)};
\addplot+ [black, dotted, opacity=0.7, domain=0:3.0] {3*x}; \addlegendentry{$m_f=m_e=0$};
\end{groupplot}
\end{tikzpicture}
...
\begin{axis}[
	...
	legend cell align=left, legend columns=1, legend transposed,
]
...
\item As confirmed by \cref{fig:lsm3f:3-flavor-density-ratios}, each quark density converges towards a third of the total baryon density as the density increases.
      ...
      \TODO{The baryon density is shown in units of the nuclear saturation density $n_\text{sat} = \SI{0.165}{\per\femto\meter\cubed}$ at which nuclei begin to overlap. \cite{ref:saturation_density}
      ...
      \TODO{I got it wrong in 2F: there is a TRANSITION at 4-7 n0, not necessarily this where it is valid.}
...
\begin{align}
	...
	B^\frac14 < \SI{112.0}{\mega\electronvolt}           \quad \Big(\text{or } \big(B-\pot_0\big)^\frac14 > \SI{226.4}{\mega\electronvolt}\Big) \qquad \Big(m_\sigma = \SI{600}{\mega\electronvolt}\Big), \label{eq:lsm3f:bag_upper_bound_600} \\
	B^\frac14 < \phantom{0}\SI{68.2}{\mega\electronvolt} \quad \Big(\text{or } \big(B-\pot_0\big)^\frac14 > \SI{231.4}{\mega\electronvolt}\Big) \qquad \Big(m_\sigma = \SI{700}{\mega\electronvolt}\Big), \label{eq:lsm3f:bag_upper_bound_700} \\
	B^\frac14 < \phantom{0}\SI{27.0}{\mega\electronvolt} \quad \Big(\text{or } \big(B-\pot_0\big)^\frac14 > \SI{241.3}{\mega\electronvolt}\Big) \qquad \Big(m_\sigma = \SI{800}{\mega\electronvolt}\Big). \label{eq:lsm3f:bag_upper_bound_800}
\end{align}%
...
%\addlegendentry{$N_f=2$};
\addplot+ [solid, mesh] table [x=R, y=M, meta expr={log10(\thisrow{P}*e0)}] {../code/data/LSM3F/stars_sigma_700_B14_68.dat};  % node [Bpin, pos=0.920] {$B = (\SI{27}{\mega\electronvolt})^4$};
...
%\addlegendentry{$N_f=2$};
\addplot+ [solid, mesh] table [x=R, y=M, meta expr={log10(\thisrow{P}*e0)}] {../code/data/LSM3F/stars_sigma_800_B14_27.dat}; % node [Bpin, pos=0.920] {$B = (\SI{27}{\mega\electronvolt})^4$};
...
\caption{\label{fig:lsm:3-flavor-mass-radius}%
	Mass-radius solutions of the Tolman-Oppenheimer-Volkoff equations \eqref{eq:tov:tovsys} parametrized by the central pressure $P_c$, obtained with the equations of state for two-flavor quark matter in \cref{fig:lsm:3-flavor-eos-eos} modified by a range of bag constants above the bounds \eqref{eq:lsm:bag_lower_bound}.
}
...
\end{groupplot}
\node (title) at ($(group c1r1.north)!0.5!(group c2r1.north)$) [above, yshift=\pgfkeysvalueof{/pgfplots/every axis title shift}] {Maximum mass star profile ($m_\sigma=\SI{800}{\mega\electronvolt}$, $B^\frac14 = \SI{27}{\mega\electronvolt}$, $P_c=10^{34.602} \, \si{\pascal}$)};
\end{tikzpicture}
...
\item The masses and radii are generally comparable to the two-flavor results, except that the mass is lowered for stars with strange quarks.
      ...
      we find masses $1.6 \leq M \leq 1.7 \, M_\odot$ and radii $\SI{11}{\kilo\meter} \leq R \leq \SI{12}{\kilo\meter}$ depending on the precise mass $m_\sigma$.
...
\TODO{move this to MIT, to introduce stiffness in the easiest chapter?}
