...
and the theory is again renormalizable.
...
also shown in \cite{ref:lsm3f} and \cite{ref:lsm3f_details}.
In order to keep the two-flavor and three-flavor models as similar and comparable as possible,
we do not include it here.
In particular, they find that it drives the chiral transition closer to being a discontinuous phase transition rather than a crossover.
...
\emph{its minimum $\avg{\sigma_x} \neq f_\pi$ moves away} and generates a \emph{different} mass $m_x = g \avg{\sigma_x} / 2$.
...
But it does not behave in this unpredictable way, and it also permits using the same values of $m_\sigma$ as in \cref{chap:lsm2f} without breaking the potential.
...
so we can only hope that the same remains true with three flavors.
...
\item For stars with greater central pressure, $\mu$ reaches sufficient levels and the strange quark is present in the core.
      ...
      This is caused by the softening of the equation of state as it ramps up during the strange crossover.
...
\item The maximum central chemical potential is comparable in the two-flavor and three-flavor models.
      ...
      For physical and stable stars, we therefore only need to include the three lightest quarks.
...
\end{itemize}
Since these pure quark stars consist of two-flavor quark matter out to the surface
and this is unstable with respect to hadronic matter,
...
The only way of achieving this with our model is to increase the bag constant.
We have checked that for all $m_\sigma$ considered,
roughly $B^\frac14 \gtrsim \SI{145}{\mega\electronvolt}$ is required for the $s$ quark to survive out to the surface.
Unfortunately, this exceeds the upper bound \eqref{eq:lsm3f:bag_upper_bound} \emph{also} due to the strange matter hypothesis,
so it is not possible to self-consistently model pure strange quark stars with this model!
\TODO{les, plot a SQS?}
...
using $\SI{600}{\mega\electronvolt} \leq m_\sigma \leq \SI{800}{\mega\electronvolt}$ and the lowest instability-respecting bag constants.
...
Unfortunately, we do not find it possible to model realistic pure quark stars with this model.
The stars we found consisted of unstable two-flavor quark matter out to the surface.
For them to be stable, the strange quark must exist out to the surface and the strange matter hypothesis must be true.
This could only be achieved in this model by
increasing the bag constant beyond its upper bound -- \emph{also} due to the strange matter hypothesis and hence contradicting it.
