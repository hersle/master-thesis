...
Like the two-flavor model, this model takes chiral symmetry breaking of quantum chromodynamics into account,
but treats confinement only phenomenologically with a bag constant $B$.
...
we will see that the three-flavor model presents us with a multitude of ways of fitting the parameters,
so that some experimental values must be predicted rather than fit.

Due to the heavier mass of the strange quark,
we expect its effects to become apparent in the mass-radius diagram only for stars that have a large central pressure and thus large quark chemical potential.
...
\TODO{comment from meeting: vacuum have same quantum numbers $=$ vacuum has zero charge, zero isospin etc., must therefore have $\avg{\bar{u} d} = 0$ etc but $\avg{\bar{u} u} \neq 0$. could also be that $\avg{\bar{u} d}$ corresponds to pion condesate or something, but this is really $\avg{\bar{u} \gamma^5 d}$ or something? finally, choice between $h_{0/3/8}$ corresponds to have up, down and strange quark symmetry is broken (for example if all three are treated separately if all three are nonzero)}
...
\end{groupplot}
\node (title) at ($(group c1r1.north)!0.5!(group c2r1.north)$) [above, yshift=\pgfkeysvalueof{/pgfplots/every axis title shift}] {\goldenstar Maximum mass star profile ($m_\sigma=\SI{800}{\mega\electronvolt}$, $B^\frac14 = \SI{27}{\mega\electronvolt}$, $P_c=10^{34.602} \, \si{\pascal}$)};
\end{tikzpicture}
