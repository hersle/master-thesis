...
\begin{equation}
\begin{split}
	\Omega(\avg{\sigma},\mu) &= \pot(\avg{\sigma},0) + N_c \frac{m_x^4}{8 \pi^2} \Bigg[ \frac{3}{2} + \log\bigg(\frac{\Lambda_x^2}{m_x^2}\bigg) \Bigg] + N_c \frac{m_y^4}{16 \pi^2} \Bigg[ \frac{3}{2} + \log\bigg(\frac{\Lambda_y^2}{m_y^2}\bigg) \Bigg] \\ % + N_c \smashoperator{\sum_{\vphantom{\big|}f=\{u,d,s\}}} \frac{m_f^4}{16 \pi^2} \left[ \frac{1}{\epsilon} + \frac{3}{2} + \log\left(\frac{{\Lambda}^2}{m_f^2}\right) \right] \\
	                         &-\smashoperator{\sum_{f=1}^{N_c}} \frac{N_c}{24 \pi^2} \left[ \left( 2 \mu_f^2 - 5 m_f^2 \right) \mu_f \sqrt{\mu_f^2 - m_f^2} + 3 m_f^4 \asinh \left( \sqrt{\frac{\mu_{\smash{f}}^2}{m_f^2}-1} \right) \right] \\
...
\item There is a strange crossover from roughly $\mu \approx \SI{370}{\mega\electronvolt}$ (for $m_\sigma=\SI{800}{\mega\electronvolt}$).
      It begins at a value of $\mu_s = m_y = \SI{410}{\mega\electronvolt}$ (for $m_\sigma=\SI{800}{\mega\electronvolt}$)
      \emph{below} its vacuum mass $\SI{429}{\mega\electronvolt}$
...
\item As confirmed by \cref{fig:lsm3f:3-flavor-density-ratios}, each quark density converges towards a third of the total baryon density as the density increases.
      ...
      \TODO{add 600 or use instead of 700}
...
\caption{\label{fig:lsm:3-flavor-mass-radius}%
	Mass-radius solutions of the Tolman-Oppenheimer-Volkoff equations \eqref{eq:master_intro:tov} parametrized by the central pressure $P_c$, using the equations of state for three-flavor quark matter in \cref{fig:lsm:3-flavor-eos-eos} modified by a range of bag constants above the bounds \eqref{eq:lsm:bag_lower_bound}.
}
