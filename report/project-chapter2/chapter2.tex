...
\begin{subequations}
\begin{align}
	%\text{the entropy}                     \quad \Avg{S}   &= -\pdv{\Omega}{T} \\
	& \text{the average number of particles} & \Avg{N_i} & = k_B T \pdv{\log{Z}}{\mu_i},                    & \label{eq:tft:average_number} \\
	& \text{the average energy}              & \Avg{E}   & = \mu_i \Avg{N_i} - \pdv{\log{Z}}{\beta}, & \label{eq:tft:average_energy} \\ % \Omega + T S + \mu_i \Avg{N_i} \\
	& \text{the         pressure}            &             P    & = \frac{k_B T}{V} \log{Z}.                       & \label{eq:tft:average_pressure} % -\pdv{\Omega}{V}
\end{align}%
...
\section{Path integral for the bosonic partition function}
...
\section{Path integral for the fermionic partition function}
...
\section{The partition function for free real scalar bosons}
...
\section{The partition function for free Dirac fermions}
...
\begin{equation}
	...
	\qquad \text{and} \qquad
	\gamma^i = \begin{bmatrix} 0 & \tau^i \\ -\tau^i & 0 \\ \end{bmatrix} ,
\label{eq:tft:gamma_dirac_basis}
\end{equation}
where $\tau^i$ are the \textbf{Pauli matrices}
\begin{equation}
	\tau^1 = \begin{bmatrix} 0 & 1 \\ 1 & 0 \\ \end{bmatrix} ,
	\qquad
	\tau^2 = \begin{bmatrix} 0 & -i \\ i & 0 \\ \end{bmatrix}
	\qquad \text{and} \qquad
	\tau^3 = \begin{bmatrix} 1 & 0 \\ 0 & -1 \\ \end{bmatrix} .
...
\begin{equation}
	D_n(\vec{p}) = \begin{bmatrix}
	                   -E_n - i \mu + i m c^2         & i c \, \vec{\tau} \cdot \vec{p}    \\ 
	                   i c \, \vec{\tau} \cdot \vec{p} & -E_n - i \mu - i m c^2 \\ 
	               \end{bmatrix} .
...
we then have $AD = (E_n + i \mu)^2 + (mc^2)^2$ and $BC = - (\vec{\tau} \cdot \vec{p})^2 c^2$.
From the Pauli matrices \eqref{eq:tft:pauli_matrices}, it is straightforward to verify the property $(\vec{\tau} \cdot \vec{p})^2 = \vec{p}^2$.
...
\begin{split}
	\tdet D  & = \tdet \left\{ -\beta \hbar \omega_n - i \beta \mu + i \beta m c^2 \gamma^0 + i \beta c \gamma^0 \vec{\gamma} \cdot \vec{p} \right\} \\
	         & = \tdet \begin{pmatrix} 
	                 -\beta \hbar \omega_n - i \beta \mu + i \beta m c^2           & i \beta c \vec{\tau} \cdot \vec{p}                \\ 
	                 i \beta c \vec{\tau} \cdot \vec{p}                          & -\beta \hbar \omega_n - i \beta \mu - i \beta m c^2 \\ 
	             \end{pmatrix} \\
	         & = \tdet \left\{ (-\beta \hbar \omega_n - i \beta \mu + i \beta mc^2) (-\beta \hbar \omega_n - i \beta \mu - i \beta m c^2) + \beta^2 c^2 (\vec{\tau} \cdot \vec{p})^2 \right\} \\
	         & = \tdet \left\{ (-\beta \hbar \omega_n - i \beta \mu)^2 + (\beta m c^2)^2 + \beta^2 c^2 \underbrace{(\vec{\tau} \cdot \vec{p})^2}_{\vec{p}^2} \right\} \\
...
The logarithm of the partition function \eqref{eq:tft:fermionic_partition_function_before_log} is now the sum
\begin{equation}
\begin{split}
...
Having evaluated $\log Z$ for both a real scalar field and Dirac fermions, it is insightful to conclude this chapter by comparing some similarities and differences of the two cases:
\begin{itemize}
