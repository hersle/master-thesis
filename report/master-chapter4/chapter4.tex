...
\TODO{try two-flavor hybrid star! merge results into text.}
...
\nextgroupplot[
	...
	title={Two-flavor hybrid equation of state },
...
\end{groupplot}
\end{tikzpicture}
\caption{\label{fig:hybrid:eos-2}%
Hybrid equation of state obtained
by combining the two-flavor quark-meson model (with $m_\sigma=\SI{600}{\mega\electronvolt}$ and $B^\frac14 = \SI{111}{\mega\electronvolt}$) for the core
...
\nextgroupplot[
	...
	xmin=900, xmax=1700, restrict x to domain=800:2100, 
	ymin=0, ymax=0.4, restrict y to domain=0:5,
	title={Three-flavor hybrid equation of state },
	title style={yshift=+0.9cm},
	legend style={anchor=south, at={(0.5,1.04)}}, legend columns=3,
];
...
\end{groupplot}
\end{tikzpicture}
\caption{\label{fig:hybrid:eos-3}%
Hybrid equation of state obtained
by combining the three-flavor quark-meson model (with $m_\sigma=\SI{600}{\mega\electronvolt}$ and $B^\frac14 = \SI{111}{\mega\electronvolt}$) for the core
with the Akmal-Pandharipande-Ravenhall hadronic equation of state for the outer region,
according to steps \ref{step:hybrid:one}--\ref{step:hybrid:four} in \cref{sec:hybrid:construction}.
}
\end{figure}
...
\addplot [yellow, opacity=0.5] fill between [of=J0348lo and J0348hi]; \addlegendentry{PSR J0348$+$0432};
...
\addplot [green, opacity=0.5] fill between [of=J1614lo and J1614hi]; \addlegendentry{PSR J1614$-$2230};
...
\addplot [name path=J0348lo, draw=none, forget plot, domain=0:40] {2.01 - 0.04};
\addplot [name path=J0348hi, draw=none, forget plot, domain=0:40] {2.01 + 0.04};
\addplot [yellow, opacity=0.5] fill between [of=J0348lo and J0348hi];

\addplot [name path=J1614lo, draw=none, forget plot, domain=0:40] {1.91 - 0.02};
\addplot [name path=J1614hi, draw=none, forget plot, domain=0:40] {1.91 + 0.02};
\addplot [green, opacity=0.5] fill between [of=J1614lo and J1614hi];
...
\caption{\label{fig:hybrid:mass-radius-2}%
	...
	\TODO{discuss bands in text. any EOS must come over this magical limit}
}
...
\end{groupplot}
\node (title) at ($(group c1r1.north)!0.5!(group c2r1.north)$) [above, xshift=-0.25cm, yshift=\pgfkeysvalueof{/pgfplots/every axis title shift}] {\goldenstar Maximum mass star profile ($N_f=2$, $m_\sigma=\SI{600}{\mega\electronvolt}$, $B^\frac14 = \SI{111}{\mega\electronvolt}$, $P_c=10^{34.59} \, \si{\pascal}$)};
\end{tikzpicture}
\caption{\label{fig:hybrid:star-2}%
	...
	for the maximum mass hybrid star \goldenstar in \cref{fig:hybrid:mass-radius-2}.
	\TODO{update text with new properties}
}
...
\caption{\label{fig:hybrid:mass-radius-3}%
	Mass-radius solutions of the Tolman-Oppenheimer-Volkoff equations \eqref{eq:tov:tovsys} parametrized by the central pressure $P_c$,
	obtained with three hybrid equations of state like the one in \cref{fig:hybrid:eos} with different values of $m_\sigma$ and $B$ in the three-flavor quark-meson model.
	The bands show measured masses of the two pulsars PSR J1614$-$2230 and PSR J0348$+$0432 from \cite{ref:antoniadis,ref:arzoumanian}.
	\TODO{discuss bands in text. any EOS must come over this magical limit}
...
\end{groupplot}
\node (title) at ($(group c1r1.north)!0.5!(group c2r1.north)$) [above, xshift=-0.25cm, yshift=\pgfkeysvalueof{/pgfplots/every axis title shift}] {\goldenstar Maximum mass star profile ($N_f=3$, $m_\sigma=\SI{600}{\mega\electronvolt}$, $B^\frac14 = \SI{111}{\mega\electronvolt}$, $P_c=10^{34.51} \, \si{\pascal}$)};
\end{tikzpicture}
\caption{\label{fig:hybrid:star-3}%
	Radial profiles for the
	pressure $P$,
	energy density $\epsilon$,
	cumulative mass $m$,
	baryon chemical potential $\mu_B$
	and particle densities $n_i$
	for the maximum mass hybrid star \goldenstar in \cref{fig:hybrid:mass-radius-3}.
	\TODO{update text with new properties}
}
...
we obtain the mass-radius curves in \cref{fig:hybrid:mass-radius-3}:
\begin{itemize}
...
\item At first glance, the new branches seem to plummet in mass straight after the branch point and indicate that all the hybrid stars are unstable.
      However, close inspection of the right panel shows that the branch with $m_\sigma = \SI{600}{\mega\electronvolt}$
      begins with a \emph{tiny} segment of stable stars whose masses increase up to the maximum $1.90 \, M_\odot$.
      The branches with greater values of $m_\sigma$ go straight downhill and have unstable stars.
...
We take a detailed look at the one for $m_\sigma=\SI{600}{\mega\electronvolt}$ and $B^\frac14 = \SI{111}{\mega\electronvolt}$ in \cref{fig:hybrid:star-3}:
\begin{itemize}
\item The quark core has a central baryon density $n_B = 6 n_\text{sat}$ and extends $R'=\SI{1.6}{\kilo\meter} = 0.14 R$ from the center with a mass of only $M' = m(R') = 0.02 M_\odot = 0.01 M$.
      It contains three-flavor quark matter with up, down and strange quarks.
\item The phase transition from quark to hadronic matter takes place at $R'=\SI{1.6}{\kilo\meter}$ and entails big drops $\Delta\epsilon = \SI{0.3}{\giga\electronvolt\per\femto\meter\cubed}$ and $\Delta n_B = 1.5 n_\text{sat}$ in energy and baryon density.
...
\item The hadronic phase dominates, making up $\SI{99.7}{\percent}$ of the star's volume and $\SI{99.0}{\percent}$ of its mass.
\end{itemize}

\section{Summary}

\TODO{continue updating with new 600 MeV stuff}
...
We saw that the hadronic mass-radius relation developed a very short branch of stable hybrid stars up to a maximum mass $M \leq 1.90 \, M_\odot$ for $m_\sigma=\SI{600}{\mega\electronvolt}$ in the quark-meson model,
but a discontinuous transition between the two phases destabilized the stars immediately thereafter.
For larger values between $\SI{600}{\mega\electronvolt} \leq m_\sigma \leq \SI{800}{\mega\electronvolt}$,
the hybrid branches realized maximum masses $1.90 M_\odot \lesssim M \lesssim 2.07 M_\odot$.
