...
\node at (0.241, 0.762) [circle, fill, inner sep=1.5pt, label={[anchor=north west, font=\small]below:$\big(P^0,\epsilon_H(P^0)\big)$}] {};
\node at (0.241, 0.953) [circle, fill, inner sep=1.5pt, label={[anchor=south east, font=\small]above:$\big(P^0,\epsilon_Q(P^0)\big)$}] {};
\end{groupplot}
\end{tikzpicture}
\end{figure}
...
\node at (0.202, 0.713) [circle, fill, inner sep=1.5pt, label={[anchor=north west, font=\small]below:$\big(P^0,\epsilon_H(P^0)\big)$}] {};
\node at (0.202, 1.020) [circle, fill, inner sep=1.5pt, label={[anchor=south east, font=\small]above:$\big(P^0,\epsilon_Q(P^0)\big)$}] {};
\end{groupplot}
\end{tikzpicture}
...
\caption{\label{fig:hybrid:mass-radius}%
	Mass-radius solutions \TODO{relations? everywhere?} of the Tolman-Oppenheimer-Volkoff equations \eqref{eq:tov:tovsys} parametrized by the central pressure $P_c$,
	...
	The bands show measured masses of the heavy pulsars PSR J1614$-$2230, PSR J0348$+$0432 and PSR J0740$+$6620 from \cite{ref:antoniadis,ref:arzoumanian,ref:fonseca}.
}
\end{figure}
...
\item Although we have not shown an explicit calculation here,
      note that the hybrid stars we get with the inconsistently fit two-flavor grand potential using $m_\sigma = \{600,700,800\} \, \si{\mega\electronvolt}$
      could equally well be generated with the consistently fit grand potential in \cref{sec:lsm2f:refinement} using $m_\sigma = \{400,500,600\} \, \si{\mega\electronvolt}$,
      which are compatible with the measurements $\SI{400}{\mega\electronvolt} \leq m_\sigma \leq \SI{550}{\mega\electronvolt}$.
\end{itemize}
...
It should be mentioned that increasing the bag constant beyond its minimal stability-respecting values we have considered here
could allow for longer branches of stable stars, but of smaller maximum masses.
Increasing $B$ in the quark equation of state would shift $P_Q(\mu) \rightarrow P_Q(\mu) - B$,
moving the intersection point in the upper panels of \cref{fig:hybrid:eos} to the left and a lower quark chemical potential.
Given more time, we could investigate this by fixing $m_\sigma$ while varying $B$, as done in \cite{ref:lsm3f_hybrid_stars}, for example.
\TODO{also add to outlook?}
