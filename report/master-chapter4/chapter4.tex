...
\caption{\label{fig:hybrid:mass-radius}%
	...
	using different values of $m_\sigma$ and the corresponding lowest bag constants $B$ above the lower bound \eqref{eq:lsm:bag_lower_bound} in the quark-meson model.
	The bands show measured masses of three heavy pulsars from \cite{ref:antoniadis,ref:arzoumanian,ref:fonseca}.
}
\end{figure}
...
\end{groupplot}
\node (title) at ($(group c1r1.north west)!0.5!(group c2r1.north east)$) [above, yshift=\pgfkeysvalueof{/pgfplots/every axis title shift}] {Consistently fit two-flavor mass-radius solutions};
\end{tikzpicture}
