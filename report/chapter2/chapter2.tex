...
Conversely, if we choose $1$ in the first, we \emph{must} choose $-\psi_j \creat_j$ in the second to ``saturate'' the product \eqref{eq:tft:integral_saturated}.
...
Again, the details of the transition to the final line are the same as for \cref{eq:tft:completeness_grassmann_discrete}.
...
The partition function \eqref{eq:tft:partition_function} follows from the trace \eqref{eq:tft:trace_grassmann} with $\smash[b]{\hat{A}} = \smash[b]{e^{-\beta (\hat{H} - \mu \hat{N})}}$ as
\begin{equation}
