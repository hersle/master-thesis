...
We will now develop a path integral for fermionic fields.
Consider a fermionic field $\psi(\vec{x})$, for example the Dirac field with the Lagrangian $\lagr = \bar{\psi} (i \hbar c \slashed\partial - m c^2) \psi$ .
Here, the conjugate momentum is $\pi = \fdv{\lagr}/{\dot\psi} = i \hbar \psi^\dagger$, so we are instructed to treat the field $\psi$ and its conjugate $i \hbar \conj\psi$ separately.
This is not a peculiarity of fermionic fields -- for a complex scalar field, for example, we would also be instructed to treat $\phi$ and $\conj\psi$ separately. 
...
With fermions, however, things are not as simple.
Fermionic field operators do not obey the bosonic commutation relation \eqref{eq:tft:boson_field_commutators}, but rather the fermionic \textbf{anticommutation relations}
\begin{equation}
\begin{split}
...
\end{equation}
A distinguishing feature of these anticommutation relations is that it requires their eigenvalues to \emph{anticommute}.
For example,
\begin{equation}
\begin{split}
...
In the derivation of the bosonic path integral \eqref{eq:tft:bosonic_partition_function}, we relied heavily on using the eigenstates of the system to take the trace in \eqref{eq:tft:partition_function}, insert completeness relations and take the inner product between eigenstates.
To find analogous ways of doing this with fermionic fields, we will consider \textbf{fermionic coherent states}.
...
Following the discussion around \cref{eq:tft:eigenvalues_must_anticommute} and the definition of Grassmann numbers in \cref{chap:grassmann_numbers}, we have, by assumption, the anticommutators
\begin{equation}
\begin{split}
	\acomm{\psi_i}{\psi_j} = \acomm{\destr_i}{\destr_j} = \acomm{\creat_i}{\creat_j} = \acomm{\psi_i}{\destr_j} = \acomm{\psi_i}{\creat_j} &= 0 , \\
	                                                                               \acomm{\destr_i}{\creat_j} = \acomm{\creat_j}{\destr_i} &= \delta_{i j} . \\
\end{split}
\label{eq:tft:fermionic_all_anticommutators}
\end{equation}
The anticommutators of the field operators $\creat_i$ and $\destr_i$ are the discrete analogues of \eqref{eq:tft:fermion_anticommutators}.
It will be less painful to describe the formalism of fermionic coherent states in the discrete case and then take the continuum limit in the end.

The system has a \textbf{ground state} or \textbf{vacuum state} that we denote by
\begin{equation}
	\ket{0, 0, \ldots, 0} = \ket{0}. \\
\end{equation}
We can apply any sequence of $n = \sum_i n_i$ \emph{different} creation operators to it to build an \textbf{$n$-particle state}
\begin{equation}
...
\end{equation}
Note that $\creat_i \ket{1_i} = (\creat_i)^2 \ket{0} = -(\creat_i)^2 \ket{0} = 0$, also by the anticommutator \eqref{eq:tft:fermionic_all_anticommutators}.
Then we have
\begin{equation}
	\destr_i \ket{1_i} = \destr_i \creat_i \ket{0} = (1 - \creat_i \destr_i) \ket{0} = \ket{0} .
\end{equation}
The interpretation of this analysis is that the \textbf{creation operators} $\creat_i$ and \textbf{destruction operators} $\destr_i$ respectively create and destroy particles when applied to the state \eqref{eq:tft:fermionic_many_particle_state}.
The restriction that $n_i \in \{0, 1\}$ implies that there can be at most one particle in the every one-particle state of the total $n$-particle state and is consistent with the Pauli exclusion principle.
In particular, note that the system has a total of $2^N$ distinct states
\begin{equation}
...
\end{equation}
But we also have $\psi_j \ket{\psi} = \psi_j \prod_i (1 - \psi_i \creat_i) \ket{0} = \psi_j \ket{0}$.
...
This is analogous to the bosonic eigenequation \eqref{eq:tft:field_eigenvalue_equations}, and it is the coherent states that we wish to use when taking the trace, inner product and inserting identity operators in the path integral.
