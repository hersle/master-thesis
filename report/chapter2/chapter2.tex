...
For fermions, commutators are replaced by anticommutators, and so eigenvalues no longer commute, but anticommute! \TODO{show this in more detail}
...
\TODO{note that we would get the same behavior with a complex scalar field}
...
Define the
\begin{equation}
	\text{vacuum state} \quad \ket{0}
	\qquad \text{one-particle state} \qquad
	\ket{1} = \creat \ket{0} .
\end{equation}
Then
\begin{equation}
	\destr \ket{1} = \destr \creat \ket{0} = (1 - \creat \destr) \ket{0} = \ket{0}
\end{equation}
\TODO{show $a \ket{0} = 0$}
and
\begin{equation}
	\creat \ket{1} = \creat \creat \ket{0} = 0 .
\end{equation}
Define the \textbf{coherent state}
\begin{equation}
	\ket{\psi} = e^{-\psi \creat \ket{0}} = ( 1 - \psi \creat ) \ket{0}
\end{equation}
Then
\begin{equation}
	\destr \ket{\psi} = \psi \ket{0} = \psi \ket{\psi} \quad \text{(last because $\psi^2=0$)}
\end{equation}
Inner product
\begin{equation}
\begin{split}
	\braket{\psi | \psi'} &= \braket{0 | (1 - \destr \conj\psi) (1 - \psi' \creat) | 0} \\
	                      &= \braket{0 | 0} + \braket{1 | \conj\psi \psi' | 1} \\
						  &= 1 + \conj\psi \psi' = e^{\conj\psi \psi'}
\end{split}
\end{equation}

Now the unit operator is
\begin{equation}
\begin{split}
	\int \dif \conj{\psi} \int \dif \psi \, e^{-\conj\psi \psi} \ket{\psi} \bra{\psi} &= \int \dif \conj{\psi} \int \dif \psi \, (1 - \conj{\psi} \psi) (1 - \psi \hat\psi^\dagger) \ket{0} \bra{0} (1 - \hat\psi \conj{\psi}) \\
	                                                                                  &= \int \dif \conj{\psi} \int \dif \psi \left[ (-\conj{\psi} \psi) \ket{0} \bra{0} + (-\psi \hat\psi^\dagger) \ket{0} \bra{0} (-\hat\psi \conj{\psi}) \right] \\
	                                                                                  &= \int \dif \conj{\psi} \int \dif \psi \left[ \psi \conj{\psi} \ket{0} \bra{0} + \psi \conj{\psi} \ket{1} \bra{1} \right] \\
	                                                                                  &= \ket{0} \bra{0} + \ket{1} \bra{1} = \hat{1} . \\
\end{split}
\end{equation}
The trace operation is
\begin{equation}
\begin{split}
	\int \dif \conj{\psi} \int \dif \psi \, e^{-\conj{\psi} \psi} \braket{-\psi | \hat{A} | \psi} &= \int \dif \conj{\psi} \int \dif \psi \, (1 - \conj{\psi} \psi) \braket{0 | (1 + \hat\psi \conj{\psi}) \hat{A} (1 - \psi \hat\psi^\dagger) | 0} \\
	                                                                                              &= \int \dif \conj{\psi} \int \dif \psi \left[ (-\conj{\psi} \psi) \braket{0 | \hat{A} | 0} + \braket{0 | \hat\psi \conj{\psi} \hat{A} (-\psi \hat\psi^\dagger) | 0} \right] \\
	                                                                                              &= \int \dif \conj{\psi} \int \dif \psi \left[ \psi \conj{\psi} \braket{0 | \hat{A} | 0} + \psi \conj{\psi} \braket{1 | \hat{A} | 1} \right] \\
																				                  &= \braket{0 | \hat{A} | 0} + \braket{1 | \hat{A} | 1} = \trace{\hat{A}} . \\
\end{split}
\end{equation}
where we assumed $\hat{A}$ to be a ``bosonic'' operator such as the Hamiltonian \TODO{?}.

A more general situation is, instead of considering one $\psi$, multiple $\psi_i$ together with multiple creation and annihilation operators $\hat\psi_i$.
In this case, we instead define the coherent state
\begin{equation}
	\ket{\psi} = e^{-\sum_i \psi_i \hat\psi_i^\dagger} \ket{0} .
\end{equation}
The unit operator turns out to be
\begin{equation}
	\int \dif \conj\psi \int \dif \psi \, e^{-\sum_i \conj\psi_i \psi_i} \ket{\psi} \bra{\psi} = \hat{1} ,
\end{equation}
while the trace becomes
\begin{equation}
	\int \dif \conj\psi \int \dif \psi \, e^{-\sum_i \conj\psi_i \psi_i} \braket{-\psi | \hat{A} | \psi} = \trace{A}
\end{equation}

But we are still not quite there.
We have \emph{functions} $\psi(\vec{x})$.
In the functional generalization, we rather define the coherent state
\begin{equation}
	\ket{\psi} = e^{-\int \dif^3 x\, \psi(\vec{x}) \hat\psi^\dagger(\vec{x})} \ket{0} .
\end{equation}
The unit operator is then
\begin{equation}
	\int \dif \conj\psi \int \dif \psi \, e^{-\int \dif^3 x \, \conj\psi(\vec{x}) \psi(\vec{x})} \ket{\psi} \bra{\psi} = \hat{1} ,
\end{equation}
and the trace is
\begin{equation}
	\int \dif \conj\psi \int \dif \psi \, e^{-\int \dif^3 x \, \conj\psi(\vec{x}) \psi(\vec{x})} \braket{-\psi | \hat{A} | \psi} = \trace{A} .
\end{equation}
...
\end{equation}
Second, the trace is \TODO{use one or two integral signs?}
\begin{equation}
...
\end{equation}
Finally, take the continuum limit and integrate $\int \dif \tau \, \dot{\psi^\dagger} \psi = -\int \dif \tau \, \psi^\dagger \dot{\psi}$ by parts \TODO{why is this valid for Grassmann numbers? Can one e.g. show that $\pdv{\phi^\dagger \phi}{\tau} = 0$?}.
