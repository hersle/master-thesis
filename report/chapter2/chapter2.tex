...
\end{equation}
Thus, the Lagrangian is invariant up to the surface term $\partial_\mu (\lambda j^\mu)$, provided that the current is conserved according to $\partial_\mu j^\mu = 0$. \TODO{significance of this?}
...
\TODO{exercise: find $Q = \int j^0$ in QED. Set up $H' = H - \mu Q$. Then go back and find $\lagr$ by LT. Look at how the $\mu$ term appears in $\lagr$. Should be like a gauge/couple current? See Martin Spillum Grønlipage eq 2.27}

\TODO{Mention that this theory is non-Abelian?}

\TODO{for Abelian theories, we are restricted to introduce chemical potentials only for commuting components of consreved currents! relevant for quark star EoS?}
...
The field is not periodic in space, but let us for a moment pretend that it is by putting it in a fictitous box of volume $V = L^3$ and impose periodic boundary conditions.
...
\TODO{$H = \hbar \omega (a^\dagger a + \frac12)$, $\braket{0 | H | 0} = \frac12 \hbar \omega$}
