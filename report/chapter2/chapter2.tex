...
\end{equation}
But $\psi_j \ket{\psi} = \psi_j \prod_i (1 - \psi_i \creat_i) \ket{0} = \psi_j \ket{0}$ because of the anticommutation \TODO{ref} of all Grassmann numbers.
...
\end{equation}
All other terms would give zero because of $\psi_i^2 = 0$ or $\int \dif \psi_i \, 1 = 0$.
\TODO{make a ``logic branch'' tree instead?}
It follows that
\begin{enumerate}
\item if one picks the factor $1$ in the first parenthesis for some particular value of $i$,
\item then one must pick the factor $-\psi_j \creat_j$ for $j = i$ in the second parenthesis to include the factor $\psi_i$ in the integral \TODO{ref},
\item and similarly one must pick the factor $-\psi_k \creat_k$ for $k = j = i$ in the third parenthesis to include the conjugate factor $\conj\psi_i$ in the integral \TODO{ref}.
\end{enumerate}
Conversely,
\begin{enumerate}
\item if one picks the factor $\psi_i \conj \psi_i$ in the first parenthesis for some particular value of $i$,
\item then one must pick the factor $1$ for $j = i$ in the second parenthesis to avoid a factor $\psi_i^2 = 0$ in the integral \TODO{ref},
\item and similarly one must pick the factor $1$ in the third parenthesis for $k = j = i$ to avoid a factor $(\conj{\psi_i})^2 = 0$ in the integral \TODO{ref}.
\end{enumerate}
...
\emph{These terms are therefore precisely all the possible states \TODO{ref} in the system!}
...
\fi
Also, to be clear, the very last equality follows because the sum goes over all states \TODO{ref} in the system.
