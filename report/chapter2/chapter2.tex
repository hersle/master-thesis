...
Thus, we can carry out the Gaussian integrals using formula \eqref{eq:integrals:gaussian_ax2} \TODO{fix this reference (and others to it?)!} after a rescaling to obtain
\begin{equation}
\begin{split}
	Z & =
...
\item In the bosonic case, we could integrate out the conjugate momentum $\pi$, leaving the Gaussian integrals \eqref{eq:tft:boson_gaussian_integral} over \emph{one} variable that we tackled using $\int \dif x \, e^{-ax^2} \propto a^{-1/2}$.
      In the fermionic case, we could not integrate out the conjugate momentum $\pi = i \hbar \conj\psi$.
      This resulted in the Gaussian integrals \eqref{eq:tft:fermion_gaussian_integral} over \emph{two} Grassmann numbers that we handled with $\int \dif \conj\psi \int \dif \psi \, e^{-\conj\psi a \psi} = a$.
