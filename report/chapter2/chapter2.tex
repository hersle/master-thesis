...
\end{split}
\end{equation}
We now change variables from $\psi(\vec{x}, \tau)$ and $\conj\psi(\vec{x}, \tau)$ to $\psi_n(\vec{k})$ and $\conj\psi_n(\vec{k})$ as in the bosonic case \eqref{eq:tft:boson_integral_new_variables}.
...
The partition function \eqref{eq:tft:dirac_partition_function_first} then becomes 
...
\end{equation}
Do not forget that $\psi^\dagger_n(\vec{p})$ and $\psi_n(\vec{p})$ are four-vectors, sandwiching a matrix to create a scalar action in the exponent!
