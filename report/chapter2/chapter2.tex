...
Let us now build the path integral for the fermionic partition function using \cref{eq:tft:grassmann_inner_product,eq:tft:completeness_grassmann,eq:tft:trace_grassmann} as our toolbox.
...
\end{equation}
Since $\tilde\pi$ can be regarded as a constant shift of the field $\pi$, we can now use the Gaussian integral \eqref{eq:integrals:gaussian_ax2} to \emph{integrate out} the momentum. 
...
Thus, we can carry out the Gaussian integrals using formula \eqref{eq:integrals:gaussian_ax2} after a rescaling to obtain
\begin{equation}
\begin{split}
	Z & =
...
Take the derivative with respect to $E^2(\vec{p})$,
\TODO{someething iffy about taking derivative with respect to one term in the sum ?!?!?}
\begin{equation}
	\odv{\log Z}{E^2(\vec{p})} = -\frac12 \sum_n \sum_\vec{p} \frac{1}{E_n^2 + E^2(\vec{p})} .
\label{eq:tft:matsubara_sum_bosons}
\end{equation}
We can now use the Matsubara frequency sum \eqref{eq:matsum:example_result}.
The result is
\begin{equation}
	\odv{\log Z}{E^2(\vec{p})} = -\frac{\beta}{4} \sum_\vec{p} \frac{1}{E(\vec{p})} \left[ 1 + \frac{2}{e^{\beta E(\vec{p})} - 1} \right] .
\end{equation}
Having evaluated the sum, let us integrate back.
By the chain rule,
\begin{equation}
	\odv{\log Z}{E(\vec{p})} = 
	2 E(\vec{p}) \odv{\log Z}{E^2(\vec{p})} = 
	-\frac{\beta}{2} \sum_{\vec{p}} \left[ 1 + \frac{2}{e^{\beta E(\vec{p})} - 1} \right] .
\end{equation}
Integration then gives
\begin{equation}
	\log Z = \sum_\vec{p} \left\{ -\frac12 \beta E(\vec{p}) - \log \left[ 1 - e^{-\beta E(\vec{p})} \right] \right\} .
\end{equation}
...
\begin{equation}
	Z = \int \pathintdif [i \hbar \psi^\dagger] \int \pathintdif \psi
...
\TODO{consistent use of $\dagger$ and $*$}
\begin{equation}
...
Using the Grassmann exponential integral \eqref{eq:integrals:gaussian_grassmann}, we obtain
\begin{equation}
...
Using the property $(\vec{\sigma} \cdot \vec{p})^2 = \vec{p}^2$, which is straightforward to verify from the Pauli matrices \eqref{eq:tft:pauli_matrices}, and again the relativistic dispersion relation $E^2(\vec{p}) = \vec{p}^2 c^2 + (mc^2)^2$, we get the simple diagonal $2 \times 2$ matrix
...
\begin{equation}
	\odv{\log Z}{\tilde E^2(\vec{p})} = \sum_n \sum_\vec{p} \left[ \frac{1}{E_n^2 + \tilde E^2(\vec{p})} +
	                                                               \frac{1}{E_n^2 + \tilde E^2(\vec{p})} \right] .
...
\begin{equation}
	\odv{\log Z}{\tilde E^2(\vec{p})} = \sum_\vec{p} \frac{\beta}{\tilde E} \left[ 1 - \frac{1}{e^{\beta \tilde E(\vec{p})} + 1}
	                                                                                 - \frac{1}{e^{\beta \tilde E(\vec{p})} + 1} \right] .
\end{equation}
By the chain rule,
\begin{equation}
	\odv{\log Z}{\tilde E} = 2 \tilde E(\vec{p}) \odv{\log Z}{\tilde E^2(\vec{p})}
...
\TODO{control integrals and signs with Kapusta}

\TODO{sum over $\tilde E$ or $E$ in first term?}

\TODO{need $\tilde E_+$ and $\tilde E_-$}
...
\TODO{har ikke snakket muntlig om disse tilbakemeldingene, see written feedback for more context}
