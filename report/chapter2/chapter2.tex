...
\end{equation}
Since $\tilde\pi$ can be regarded as a constant shift of the field $\pi$, we can now use the Gaussian integral \eqref{eq:integrals:gaussian_sqrtpi} with a suitable rescaling of $x$ to \emph{integrate out} the momentum. 
...
Note that either of the two expansions we have written is as good as the other for our \emph{real} scalar field $\psi(\vec{x},\tau) = \conj\psi(\vec{x},\tau)$.
...
\begin{split}
	\log Z & = \sum_n \sum_\vec{p} \tdet D_n(\vec{p}) \\
	       & = \sum_n \sum_\vec{p} \log \beta^4 \left[ (E_n + i \mu)^2 + E^2(\vec{p}) \right]^2 \\
...
\begin{equation}
\begin{split}
	\log Z & = \sum_n \sum_\vec{p} \log \beta^4 \left[ (E_n + i \mu)^2 + E^2(\vec{p}) \right]^2 \\
	       & = \sum_n \sum_\vec{p} \log \beta^2 \left[ (E_n + i \mu)^2 + E^2(\vec{p}) \right] 
	         + \sum_n \sum_\vec{p} \log \beta^2 \left[ (E_n + i \mu)^2 + E^2(\vec{p}) \right] \\
	       & = \sum_n \sum_\vec{p} \log \beta^2 \left[ (E_n + i \mu)^2 + E^2(\vec{p}) \right]
	         + \sum_n \sum_\vec{p} \log \beta^2 \left[ (-E_n + i \mu)^2 + E^2(\vec{p}) \right] \\
	       & = \sum_n \sum_\vec{p} \log \beta^4 \left[ (E_n + i \mu)^2 + E^2(\vec{p}) \right] 
	                                            \left[ (-E_n + i \mu)^2 + E^2(\vec{p}) \right] . \\
\end{split}
...
\begin{equation}
\begin{split}
	\log Z & = \sum_n \sum_\vec{p} \log \beta^4 \left[ E_n^2 + (E(\vec{p}) - \mu)^2 \right] \left[ E_n^2 + (E(\vec{p}) + \mu)^2 \right] \\
