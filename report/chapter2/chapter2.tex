...
Functions of Grassmann numbers are defined by their Taylor expansion \TODO{cannot read comment. follows from or something? ask}.
The property $\psi_i^2 = 0$ will always terminate the Taylor series after a finite number of term, so the most general function can be written
\begin{equation}
\begin{split}
...
For example, we define the exponential of one Grassmann number $\psi$ by its normal Taylor series
\begin{equation}
	\exp(\psi) = 1 + \psi .
