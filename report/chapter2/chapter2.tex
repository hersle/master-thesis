...
(TODO: add chemical potential $\mu$. why should it be the conserved charge or something?)
...
\begin{equation}
	Z = \oint_- \pathintdif [i \hbar \psi^\dagger] \oint_- \pathintdif \psi \exp \left\{ \frac{1}{\hbar} \int_0^{\beta \hbar} \dif \tau \int \dif^3 x \left( -\hbar \psi^\dagger(\vec{x},\tau) \dot{\psi}(\vec{x},\tau) - \ham \left( \psi^\dagger(\vec{x},\tau), \psi(\vec{x},\tau) \right) + \mu \numdensity \right) \right\} ,
...
\end{equation}
We can now calculate the partition function \eqref{eq:tft:bosonic_partition_function}.
...
\end{split}
\end{equation}
To proceed, let us use the periodicity of the field to expand it in a Fourier series of the form \eqref{eq:tft:fourier_series}.
For our real field $\psi(\vec{x},\tau) = \psi(\vec{x},\tau)^*$, we can write
\begin{equation}
\begin{split}
...
\end{equation}
where we have pulled out a prefactor $\hbar c \sqrt{\beta/V}$ chosen to have the same dimension as the field $\phi(\vec{x}, \tau)$, so the Fourier amplitudes $\phi_n(\vec{p})$ are dimensionless.
...
(TODO: turns out it is easier to sum the derivative, then integrate it, rather than straight up summing -- do Matsubara frequency sum)

This expression may look neat enough on its own, but we can push further.
...
\end{equation}
so the Hamiltonian density is (TODO: bold vectors)
\begin{equation}
...
\end{equation}
We should replace $\numdensity$ by $j^0 = \bar\psi \gamma^0 \psi$. (TODO: why?)

Inserting the Hamiltonian density \eqref{eq:tft:dirac_hamiltonian} and the conserved density $\numdensity = j^0 = \bar\psi \gamma^0 \psi$ (TODO: why?) in the path integral \eqref{eq:tft:fermion_partition_function}, we find the fermionic partition function
\begin{equation}
...
\begin{equation}
	\psi(\vec{x}, \tau) = \frac{1}{\sqrt{V}} \sum_n \sum_\vec{p} e^{i (\vec{p} \cdot \vec{x} / \hbar + \omega_n \tau)} \psi_{n}(\vec{p})
\label{eq:tft:dirac_fourier_series}
\end{equation}
with fermionic Matsubara frequencies \eqref{eq:tft:matsubara_frequencies}.
As with the bosonic field, we have made the Fourier amplitudes $\psi(\vec{p})$ dimensionless by pulling out a factor $1/\sqrt{V}$.
...
\begin{equation}
\begin{split}
	S & = \frac{1}{V} \sum_{m,n} \sum_{\vec{p},\vec{q}}         \psi_m(\vec{q})^\dagger \gamma^0 \left[ -\hbar \gamma^0 i \omega_n + i \hbar c \vec{\gamma} \cdot i \vec{k} - m c^2 + \mu \gamma^0 \right] \psi_n(\vec{p}) \\
	  & \times \underbrace{\int_0^{\beta \hbar} \dif \tau \, e^{i(\omega_n-\omega_m)\tau}}_{\delta_{m,n} \beta \hbar} \underbrace{\int \dif^3 x \, e^{i(\vec{p}-\vec{q})\cdot\vec{x}/\hbar}}_{\delta(\vec{p} - \vec{q}) V} \\
...
\begin{equation}
	Z %& = \prod_n \prod_\vec{p} \int \pathintdif [i \hbar \psi_n(\vec{p})^\dagger] \int \pathintdif \psi_n(\vec{p}) \\
	  ...
	  \exp \left\{ \psi_n(\vec{q})^\dagger \left[ \beta( -i \hbar \omega_n - c \gamma^0 \vec{\gamma} \cdot \vec{p} - mc^2 \gamma^0 + \mu ) \right] \psi_n(\vec{p}) \right\} \\
...
\begin{equation}
	...
	D_n(\vec{p}) = [i \beta( -\hbar \omega_n + i c \gamma^0 \vec{\gamma} \cdot \vec{p} + i mc^2 \gamma^0 - i \mu )].
\end{equation}
The obvious thing to do is to try to calculate the determinant of the $4 \times 4$ matrices $D_n(\vec{p})$.
...
\begin{equation}
	D_n(\vec{p}) = i \beta \begin{bmatrix}
	                           -\hbar \omega_n - i \mu + i m c^2 & i c \vec{\sigma} \cdot \vec{p}    \\ 
	                           i c \vec{\sigma} \cdot \vec{p}    & -\hbar \omega_n - i \mu - i m c^2 \\ 
	                       \end{bmatrix} .
\end{equation}
Calculating the determinant becomes twice as easy by applying the formula
(TODO: math operator, write in determinant properly)
\begin{equation}
...
$AD = (\hbar \omega_n + i \mu)^2 + (mc^2)^2$ and $BC = - (\vec{\sigma} \cdot \vec{p})^2 c^2$.
Using the property $(\vec{\sigma} \cdot \vec{p})^2 = \vec{p}^2$ and the definition $(\hbar \omega)^2 = \vec{p}^2 c^2 + (mc^2)^2$, we get the simple diagonal $2 \times 2$ matrix
$AD - BC = (\hbar \omega_n + i \mu)^2 + \hbar^2 \omega(\vec{p})^2$.
The determinant then becomes
\begin{equation}
	\tdet D_n(\vec{p}) = \beta^4 \left[ (\hbar \omega_n + i \mu)^2 + \hbar^2 \omega^2 \right]^2 .
\end{equation}
...
\end{split}
\end{equation}
The $i$ in front of $\mu$ is awkward and can certainly not appear the physical quantities that are derived from $\log Z$.
Let us see if we can cast a magic spell and make it disappear.
The logarithm involves a sum of the type $\sum_n f(\omega_n)$ over all integers $n$.
Because the Matsubara frequencies \eqref{eq:tft:matsubara_frequencies} satisfy $\omega_n = -\omega_{-n}$, we have
$\sum_n f(\omega_n) = \sum_n f(\omega_{-n}) = \sum_n f(-\omega_n)$.
We can then split up the logarithm of the square into two logarithms and exchange $\omega_n \rightarrow -\omega_n$ in one of them.
More precisely,
\begin{equation}
\begin{split}
	\log Z & = \sum_n \sum_\vec{p} \log \beta^4 \left[ (\hbar \omega_n + i \mu)^2 + \hbar^2 \omega^2 \right]^2 \\
	       & = \sum_n \sum_\vec{p} \log \beta^2 \left[ (\hbar \omega_n + i \mu)^2 + \hbar^2 \omega^2 \right] 
	         + \sum_n \sum_\vec{p} \log \beta^2 \left[ (\hbar \omega_n + i \mu)^2 + \hbar^2 \omega^2 \right] \\
	       & = \sum_n \sum_\vec{p} \log \beta^2 \left[ (\hbar \omega_n + i \mu)^2 + \hbar^2 \omega^2 \right]
	         + \sum_n \sum_\vec{p} \log \beta^2 \left[ (-\hbar \omega_n + i \mu)^2 + \hbar^2 \omega^2 \right] \\
	       & = \sum_n \sum_\vec{p} \log \beta^4 \left[ (\hbar \omega_n + i \mu)^2 + \hbar^2 \omega^2 \right] 
	                                            \left[ (-\hbar \omega_n + i \mu)^2 + \hbar^2 \omega^2 \right] . \\
\end{split}
\label{eq:tft:dirac_partition_function_splitting}
\end{equation}
If you are reading this on paper, you can use some of the whitespace to verify that
\begin{equation}
	(\pm \hbar \omega_n + i \mu)^2 + \hbar^2 \omega^2 = (\hbar \omega_n \pm i (\omega + \mu)) (\hbar \omega_n \mp i(\omega - \mu))
\end{equation}
by expanding all parentheses.
...
\begin{equation}
\begin{split}
	\log Z & = \sum_n \sum_\vec{p} \log \beta^4 \left[ \hbar \omega_n + i (\hbar \omega + \mu) \right] 
	       ...
	       & = \sum_n \sum_\vec{p} \log \beta^2 \left[ \hbar^2 \omega_n^2 + (\hbar \omega - \mu)^2 \right]
	         + \sum_n \sum_\vec{p} \log \beta^2 \left[ \hbar^2 \omega_n^2 + (\hbar \omega + \mu)^2 \right] \\
\end{split}
\end{equation}
...
Define $\tilde \omega = \omega - \mu / \hbar$ and take the derivative
\begin{equation}
	\odv{\log Z}{\tilde \omega^2} = \sum_n \sum_\vec{p} \left[ \frac{1}{\omega_n^2 + (\omega - \mu/\hbar)^2} +
	                                                      \frac{1}{\omega_n^2 + (\omega + \mu/\hbar)^2} \right] .
\end{equation}
Then we can perform the sum over $n$ with the Matsubara frequency sum \eqref{eq:matsum:example_result} to write
\begin{equation}
	\odv{\log Z}{\tilde \omega^2} = \sum_\vec{p} \frac{\beta \hbar}{\tilde \omega} \left( 1 - \frac{1}{e^{\beta (\hbar \omega - \mu)} + 1}
	                                                                                        - \frac{1}{e^{\beta (\hbar \omega + \mu)} + 1} \right) .
\end{equation}
By the chain rule,
(TODO: missing some factor $\hbar$ here! should operate with $E = \hbar \omega$ in the appendix!)
\begin{equation}
	\odv{\log Z}{\tilde \omega} = 2 \tilde\omega \odv{\log Z}{\tilde \omega^2}
	                            = 2 \beta \hbar \sum_\vec{p} \left( 1 - \frac{1}{e^{\beta (\hbar \omega - \mu)} + 1}
	                                                                  - \frac{1}{e^{\beta (\hbar \omega + \mu)} + 1} \right) .
\end{equation}
Integrating back, we find
\begin{equation}
	\log Z = 2 \sum_\vec{p} \left( \beta \hbar \tilde\omega + \log\left[ e^{-\beta \hbar \tilde\omega}+1 \right] + \log\left[ e^{-\beta \hbar \tilde\omega} + 1\right] \right) .
\end{equation}
Writing out our continuous definition \eqref{eq:tft:continuous_limit} of $\sum_\vec{p}$, we finally obtain the \textbf{partition function for Dirac fermions}
\begin{equation}
	\log Z = 2 V \int \frac{\dif^3 p}{(2 \pi \hbar)^3} \left( \beta \hbar \tilde\omega + \log\left[ e^{-\beta \hbar \tilde\omega_+}+1 \right] + \log\left[ e^{-\beta \hbar \tilde\omega_-} + 1\right] \right) .
