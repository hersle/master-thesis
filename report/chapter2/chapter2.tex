...
Suppose we have a finite number $\psi_i$ of Grassmann numbers and equally many creation and destruction operators $\creat_i$ and $\destr_i$.
...
\begin{equation}
\begin{split}
	\text{\textbf{vacuum state}}       \quad & \ket{0} \\
	\text{and \textbf{one-particle state}} \quad & \ket{1_i} = \creat_i \ket{0} .
\end{split}
\end{equation}
Note that $\creat_i \ket{1_i} = \creat_i \creat_i \ket{0} = 0$ because $\creat_i \creat_i = 0$ by \TODO{ref}.
Then
\begin{equation}
	\destr_i \ket{1_i} = \destr_i \creat_i \ket{0} = (1 - \creat_i \destr_i) \ket{0} = \ket{0} .
\end{equation}
Similarly, $\destr_i \ket{0} = \destr_i \destr_i \ket{1_i} = 0$.
Thus, the states \TODO{ref} are all the states we can make with our creation and annihilation operators.

Next, define the \textbf{coherent state}
\begin{equation}
	\ket{\psi} = \exp \left( -\sum_i \psi_i \creat_i \right) \ket{0} = \left( 1 - \sum_i \psi_i \creat_i \right) \ket{0} .
\end{equation}
Then
\begin{equation}
	%\destr_j \ket{\psi} = \psi_j \ket{0} = \psi_j \ket{\psi} \quad \text{(last because $\psi_j^2=0$)}
	\destr_j \ket{\psi} = \destr_j \left( 1 \sum_i \psi_i \creat_i \right) \ket{0} = + \sum_i \psi_i \destr_j \creat_i \ket{0} = \psi_j \ket{0} .
\end{equation}
But $\psi_j \ket{\psi} = \psi_j (1 - \sum_i \psi_i \creat_i) \ket{0} = \psi_j \ket{0}$ because of the anticommutation \TODO{ref} of all Grassmann numbers.
Thus, the coherent states are eigenvectors of the destruction operators,
\begin{equation}
...
\begin{equation}
\begin{split}
	\braket{\psi | \psi'} &= \braket{0 | (1 - \sum_i \destr_i \conj\psi_i) (1 - \sum_j \psi'_j \creat_j) | 0} \\
...
We can work out that the unit operator $\hat{1}$ can be represented by the integral
\begin{equation}
\begin{split}
	 & \int \dif \conj{\psi} \int \dif \psi \, e^{-\sum_i \conj\psi_i \psi_i} \ket{\psi} \bra{\psi} \\
	=& \int \dif \conj{\psi} \int \dif \psi \, \left( 1 - \sum_i \conj{\psi}_i \psi_i \right) \left( 1 - \sum_j \psi_j \creat_j \right) \ket{0} \bra{0} \left( 1 - \sum_k \destr_k \conj\psi_k \right) \\
...
\end{split}
\end{equation}
To be clear, the final equality follows because $\ket{0}$ and $\ket{1_j}$ are all the states in the system.
Similarly, we can work out that the trace of an operator $\hat{A}$ can be written
\begin{equation}
\begin{split}
	 & \int \dif \conj{\psi} \int \dif \psi \, e^{-\sum_i \conj{\psi}_i \psi_i} \braket{-\psi | \hat{A} | \psi} \\
	=& \int \dif \conj{\psi} \int \dif \psi \, (1 - \sum_i \conj\psi_i \psi_i) \braket{0 | (1 + \sum_j \destr_j \conj\psi_j) \hat{A} (1 - \sum_k \psi_k \creat_k) | 0} \\
...
The above analysis was for a finite number of creation and destruction operators $\creat_i$ and $\destr_i$ accompanied by a matching number of Grassmann numbers $\psi_i$.
We are studying \emph{fields} $\psi(\vec{x})$ and $\conj\psi(\vec{x})$ with an infinite number of field operators $\creat(\vec{x})$ and $\destr(\vec{x})$ that do not satisfy the discrete anticommutation relation \TODO{ref}, but rather its continuum generalization \TODO{ref}.
