...
\begin{split}
	Z & = \int \dif \phi_0 \int \pathintdif \pi \oint_{\phi_0(\vec{x})}^{\phi_0(\vec{x})} \pathintdif \phi \, \exp \big( S_E \left[ \pi(\vec{x}, \tau), \phi(\vec{x}, \tau) \right] / \hbar \big) \\
	  & =                  \int \pathintdif \pi \oint_{\text{periodic}}                   \pathintdif \phi \, \exp \big( S_E \left[ \pi(\vec{x}, \tau), \phi(\vec{x}, \tau) \right] / \hbar \big) , \qquad \text{TODO: remove $\int \dif \phi_0$ properly!}
\end{split}
\end{equation}
