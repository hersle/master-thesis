...
\end{equation}
which is familiar for a $2 \times 2$, but in fact also valid for a block matrix where $CD = DC$, which is applicable to our case where $D$ is diagonal \cite[theorem 3]{ref:determinant_block_matrix}.
Without $i \beta$, which we take care of using $\tdet{(AB)} = \tdet{A} \tdet{B}$ and $\tdet{i \beta} = (i \beta)^4 = \beta^4$, we get \TODO{without is weird wording}
