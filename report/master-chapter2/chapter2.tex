...
\begin{equation}
	\Omega = -N_c N_f \Omega_0 - \smashoperator{\sum_{\vphantom{\big|} f=\{u,d,s\}}} \frac{N_c}{24 \pi^2} \left[ \left( 2 \mu_f^2 - 5 m_q^2 \right) \mu_f \sqrt{\mu_f^2 - m_q^2} + 3 m_q^4 \asinh \left( \sqrt{\frac{\mu_{\smash{f}}^2}{m_q^2}-1} \right) \right]
\end{equation}
...
Two-flavor grand potential \eqref{eq:lsm:grand_potential} in vacuum with $\vec{\mu}=\vec{0}$ for varying values of $m_\sigma$.}
\end{figure}
...
\TODO{elaborate that this is an ``aside'', not strictly necessary, mark with star, or something?}
...
\end{equation}
Here $\Delta$ is the dynamic quark mass while $f_\pi$, $m_q$, $m_\pi$ and $m_\sigma$ are vacuum values, and
...
Consistently fit two-flavor grand potential \eqref{eq:lsm2f:grand_potential_consistent} in vacuum with $\vec{\mu}=\vec{0}$ for varying values of $m_\sigma$.}
\TODO{plot inconsistent with dashed lines?}
\end{figure}
