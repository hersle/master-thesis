...
In contrast to the mass-\emph{conditional} chiral symmetry of quantum chromodynamics, however,
...
To see this, first express the Yukawa coupling with left-handed and right-handed chiral fields $q_\pm = P_\pm q$:
\begin{equation}
\begin{aligned}
	\bar{q} \big[\sigma + i \gamma^5 \vec{\tau} \cdot \vec{\pi}\big] q &= \bar{q} \big[ (P_+ + P_-) \sigma + (P_+ - P_-) i \vec{\tau} \cdot \vec{\pi}\big] q && \; \big( P_\pm = \tfrac{1}{2} (1 \pm \gamma^5) \big) \\
	                                                           ...
	                                                           &= \bar{q} \big[ P_+ \phi P_+ + P_- \phi^\dagger P_-\big] q && \; \big( \text{$P_\pm q = q_\pm$, $P_\pm P_\mp = 0$} \big)\\
...
\TODO{mass-conditionalness bullshit?}
\begin{equation}
...
\end{itemize}
Note how the oppositely signed parameters $m^2<0$ and $\lambda > 0$ are needed for the minimum $\avg{\sigma} \neq 0$ to exist,
...
\begin{equation}
	\lagr \taylor \sum_{f=1}^{\smash{N_f}} \sum_{c=1}^{N_c} \bar{q} \Big[ i \slashed\partial + \mu \gamma^0 - m_q \Big] q
...
\end{equation}
where the meson potential \eqref{eq:lsm:potential} to second order is
\begin{equation}
...
\item In the \textbf{chiral limit} $h = 0$,
      ...
      the $O(4)$ rotation symmetry of the four original meson fields $[\sigma,\vec\pi]^T$ is spontaneously broken
...
Practically speaking, the two different symmetry breaking patterns are equivalent and indistinguishable.
According to Weinberg's philosophy presented in \cref{sec:master_intro:qcd},
the quark-meson model is then an appropriate effective model of quantum chromodynamics at low energy.
The parameter $h \neq 0$ only serves to explicitly break the exact symmetry into an approximate one,
...
\begin{equation}
	...
	h       = m_\pi^2 \avg{\sigma}, \quad % &\qquad& \text{(by \eqref{eq:lsm:mass_pi})} , \\
...
Two-flavor grand potential \eqref{eq:lsm:grand_potential} in vacuum, where $\vec{\mu}=\vec{0}$, for different $m_\sigma$.}
\end{figure}
...
Properties of electrically charge neutral quark matter in $\beta$-equilibrium in the two-flavor quark-meson model.
...
\TODO{fix all plots, see print notes}
}
\end{figure}
...
and solve the corresponding Tolman-Oppenheimer-Volkoff equation \eqref{eq:master_intro:tov}.
...
Consistently fit two-flavor grand potential \eqref{eq:lsm2f:grand_potential_consistent} in vacuum, where $\vec{\mu}=\vec{0}$, for different $m_\sigma$
