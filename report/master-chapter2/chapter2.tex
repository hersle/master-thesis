...
Like the quantum chromodynamics Lagrangian \eqref{eq:qcd:lagrangian}, the Lagrangian \eqref{eq:lsm:lagrangian} has the vector $U(1)_V$ and axial $U(1)_A$ symmetry shown in \cref{sec:qcd:symmetry}.
...
\begin{equation}
	SU(2) \times SU(2) \simeq O(4) \quad \rightarrow \quad O(3)
\end{equation}
...
we use that the values of $\avg{\sigma}=f_\pi$, $m_\sigma$, $m_\pi$ and $m_q$ are measured and known in vacuum.
...
\end{equation}
Unfortunately, the mass $m_\sigma$ is 
The input values and output parameters are shown in \cref{tab:lsm2f:parameters}.

\TODO{continue here}

\TODO{comment on $\sigma$ mass very uncertain. rather merge into calc of grand potential, to show plots for which $m_\sigma$ it works etc.?}

\TODO{screw the table and write in text instead, to easier explain differences among the parameters?}

\TODO{move AFTER grand potential, to easier comment on inconsistent matching etc.}

\section{Grand potential and equation of state}

\section{Mass-radius solutions}

To calculate the \eqref{eq:master_intro:partition_function} we will use the mean-field approximation,
replacing the bosonic fields by their expectation values in the classical ground state
and studying the surrounding quantum fluctuations to first nonzero order.
...
\TODO{rewrite comment on choice of free variable. $\mu_u$ vs $\mu_Q$ vs $\Delta$}
