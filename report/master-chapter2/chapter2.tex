...
The integral of the middle term in the curly brackets was calculated in the zero-temperature pressure $P=-\Omega$ in equation \eqref{eq:nstars:pressure_zeroT}.
This time we also include and renormalize the infinite vacuum contribution from the integral over the preceding term.
...
\begin{equation}
	\Omega_0 = -2 \int \frac{\dif^3 p}{(2 \pi)^3} E(\vec{p})
	         = -2 \int \frac{\dif^3 p}{(2 \pi)^3} \sqrt{p^2 + m_q^2}.
...
This is also inconsistent, as the masses would really receive radiative loop corrections in this case.
...
\TODO{comment on using $\avg{\sigma}$ free instead of $\mu$ because of phase transition}
\begin{subequations}
\begin{align}
...
We parametrize solutions with one variable, say $\avg{\sigma}$,
