...
\end{equation}
\TODO{JO mail: lagr above in Mink space but has Eucl inds?}
...
the latter is \emph{unconditionally} invariant under a $SU(2)_L \times SU(2)_R$ transformation when $h=0$ and $\mu = 0$.
...
are given by minima of the meson potential $\pot(\avg{\sigma},\avg{\vec{\pi}})$ given in \eqref{eq:lsm:potential} and illustrated in \cref{fig:lsm:potential}.
...
Note that the up and down quarks have a common mass $m_u = m_d = m_q$.
...
let us set out to calculate its grand potential \eqref{eq:master_intro:grand_potential}.
...
-- see for example the study of pion condensation in the quark-meson model by \cite{ref:jo_lsm_pion_condensation,ref:jo_lsm_consistent}.
...
Although our exclusion of bosonic fluctuations is \emph{inconsistent} with respect to the number of loops considered in Feynman diagrams,
it can be regarded as \emph{consistent} in the large-$N_c$ approximation scheme.
...
we use measured values of the ground state $\avg{\sigma}=f_\pi$ and masses $m_\sigma$, $m_\pi$ and $m_q$ in vacuum.
...
In \cref{chap:lsm3f} we will see that the same happens in the three-flavor case, only then for even greater masses $m_\sigma \lesssim \SI{600}{\mega\electronvolt}$.
Since we would like to compare the results obtained with two and three flavors,
we decide to mainly operate with three common values $m_\sigma = \{600,700,800\} \, \si{\mega\electronvolt}$.
The different values will also illustrate qualitatively different behaviors of the chiral phase transition.
...
\begin{equation}
	P \rightarrow P - P(\mu \leq \SI{300}{\mega\electronvolt}) = P + \Omega(\avg{\sigma}=f_\pi) .
...
We therefore focus our attention on the bag constants that lie at the lower bounds \eqref{eq:lsm:bag_lower_bound},
as they generate greater maximum masses and arbitrarily lower masses can be reached with arbitrarily greater bag constants.
...
In \cref{fig:lsm:2-flavor-star} we take an in-depth look at the maximum mass star that has no complicating phase transition and respects the strange matter hypothesis:
\begin{itemize}
...
\item The quark chemical potential $\mu(R)$ at the surface exceeds the vacuum value $\SI{300}{\mega\electronvolt}$ because of the bag constant shift \eqref{eq:mit:bag_shift}.
\item Fortunately, the quark chemical potential $\mu(r) < \SI{490}{\mega\electronvolt}$ never exceeds $\SI{575}{\mega\electronvolt}$, beyond which we saw that neglecting pion condensation was an inconsistent approximation. % in \cref{fig:lsm:2-flavor-eos-parametrization}.
\item The crossover around $\mu \approx \SI{320}{\mega\electronvolt}$ takes place very close to the surface of the star,
