...
\TODO{potential is additional contribution to an effective medium-dependent (?) bag pressure}
...
We find the central pressure-parametrized mass-radius curves in \cref{fig:lsm:2-flavor-mass-radius}, producing maximum masses $1.7 M_\odot < M < 2.1 M_\odot$ with corresponding radii $\SI{9}{\kilo\meter} < R < \SI{12}{\kilo\meter}$ depending on the mass $m_\sigma$.
This is similar to the MIT bag model solutions in \cref{fig:mit:mass_radius}.
For $m_\sigma = \SI{800}{\mega\electronvolt}$, where we have the closed bag window \eqref{eq:lsm:bag_window_800}, the maximum mass stars have $M \approx \SI{1.76}{} M_\odot$ and $R \approx \SI{11}{\kilo\meter}$.
The maximum mass tends to increase as $m_\sigma$ is lowered to more accurate values.
\TODO{make point list out of this, too?}
...
\item Fortunately, the quark chemical potential $\mu_Q(r)$ never exceeds $\SI{575}{\mega\electronvolt}$, for which we saw that neglecting pion condensation was an inconsistent approximation in \cref{fig:lsm:2-flavor-eos-parametrization}.
...
\section{Refinements with consistent parameter fixing}
...
\TODO{try to reverse a, b in references}
...
\end{split}
\end{equation}
\TODO{make my notation consistent with this?}
...
\end{equation}
Here $f_\pi$ is the pion decay constant and $m_q$, $m_\pi$ and $m_\sigma$ are vacuum masses, and
\begin{equation}
...
\begin{axis}[
	width = 15cm, height = 7cm,
	xmin=-500, xmax=+500, xtick distance = 100, minor x tick num=9,
	ymin=-16, ymax=+6, ytick distance=5, minor y tick num=4,
	xlabel = {$\Delta \, / \, \si{\mega\electronvolt}$}, ylabel = {$\Omega \, / \, f_\pi^4$},
	legend style = {at={(0.5,1.03)}, anchor=south}, legend columns=4,
	cycle list/YlOrRd-6,
]
...
\end{axis}
\end{tikzpicture}
\caption{\label{fig:lsm:potential_sigma_mass_ref}\TODO{caption}
Two-flavor vacuum potential with
$\mu_u = \mu_d = \mu_e = 0$}
\TODO{plot non-consistent with dashed lines?}
\end{figure}
