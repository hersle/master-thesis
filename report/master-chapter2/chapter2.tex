...
\begin{subequations}
\begin{align}
	B(f_\pi)^\frac14 > \SI{110.6}{\mega\electronvolt},           \quad \text{or} \quad B(0)^\frac14 > \SI{145.4}{\mega\electronvolt} \qquad \Big(\text{with } m_\sigma = \SI{600}{\mega\electronvolt}\Big), \\
	B(f_\pi)^\frac14 > \phantom{0}\SI{67.7}{\mega\electronvolt}, \quad \text{or} \quad B(0)^\frac14 > \SI{146.3}{\mega\electronvolt} \qquad \Big(\text{with } m_\sigma = \SI{700}{\mega\electronvolt}\Big), \\
	B(f_\pi)^\frac14 > \phantom{0}\SI{27.0}{\mega\electronvolt}, \quad \text{or} \quad B(0)^\frac14 > \SI{156.5}{\mega\electronvolt} \qquad \Big(\text{with } m_\sigma = \SI{800}{\mega\electronvolt}\Big).
\end{align}%
\label{eq:lsm:bag_lower_bound}
\end{subequations}%
In the next chapter we study the three-flavor model and complement these lower bounds the by upper bounds \eqref{eq:lsm3f:bag_upper_bound} for $m_\sigma = \{700,800\}\,\si{\mega\electronvolt}$,
yielding closed bag windows.
\TODO{also for 700?}
\TODO{\cite{ref:quark_star_review} about bag constant: ``comparison of the precise values obtained in different models is not generally meaningful''}
...
\caption{\label{fig:lsm:2-flavor-mass-radius}%
	...
	\TODO{use min, max and half $B$ for 700/800, and only min $B$ for 600!}
}
\end{figure}
...
We find the central pressure-parametrized mass-radius curves in \cref{fig:lsm:2-flavor-mass-radius}, producing maximum masses $1.7 < M < 2.1 M_\odot$ with corresponding radii $\SI{9}{\kilo\meter} < R < \SI{12}{\kilo\meter}$ depending on the mass $m_\sigma$.
This is similar to the MIT bag model solutions in \cref{fig:mit:mass_radius}.

The solution of the Tolman-Oppenheimer-Volkoff equations \eqref{eq:master_intro:tov} yields the pressure, energy density and mass profiles $P(r)$, $\epsilon(r)$ and $m(r)$ of a star.
We can then invert the pressure $P(\mu)$ as a function of the quark chemical potential to find its radial profile $\mu(r)$,
and hence the particle density profiles $n_i(r)$ by looking up the corresponding value in \cref{fig:lsm:2-flavor-eos}.

\TODO{continue here on Tuesday}

\TODO{plot chemical potential as function of radius to show which phases are active!!}
...
\TODO{%
	only $m_\sigma=800$ for simplicity, dont have to think about phase transition behavior of densities etc.
	take most extreme star with largest $B$ in stability window, will show star with largest chemical potential.
}
