...
However, they are now seemingly massless and coupled to a bosonic $\sigma$ meson and three bosonic pions $\vec\pi = [\pi^+, \pi^-, \pi^0]^T$ with a Yukawa coupling $g$ and the Pauli matrices \eqref{eq:tft:pauli_matrices} \TODO{change $\sigma$ used for Pauli matrices earlier to $\tau$?}.
...
\Cref{tab:lsm2f:parameters} summarizes the chosen input values for $\avg{\sigma}$, $m_\pi$, $m_\sigma$ and $m_q$ in vacuum and the corresponding output parameters $m^2$, $\lambda$, $g$, $h$ and $\Lambda$.
...
\caption{\label{fig:lsm:grand-potential-noisospin}%
	...
	\TODO{$\avg{\sigma}$ here instead of $m_q$}
}
\end{figure}
