...
but we \TODO{others also have} will interpret the fermions as quarks instead of nucleons in what is commonly referred to as a \textbf{quark-meson model}.
\TODO{introduce (QM)? (QCD)? etc.}
...
However, they are now seemingly massless and coupled to a bosonic $\sigma$ meson and three bosonic \TODO{only say pions here and elsewhere} pions $\vec\pi = [\pi^+, \pi^-, \pi^0]^T$ with a Yukawa coupling $g$ and the Pauli matrices \eqref{eq:tft:pauli_matrices}.
The meson potential features three couplings $m^2<0$, $\lambda>0$ and $h>0$.

Like the quantum chromodynamics \TODO{QCD here and elsewhere} Lagrangian \eqref{eq:qcd:lagrangian}, the quark-meson model Lagrangian \eqref{eq:lsm:lagrangian} has a vector $U(1)_V$ and axial $U(1)_A$ symmetry shown in \cref{sec:master_intro:qcd}.
...
and noting that the meson potential \eqref{eq:lsm:potential} is invariant under rotation of this vector in $O(4)$ \TODO{really in $R^4$-space, say $O(4)$ in another way}.
...
\item In the \textbf{chiral limit} $h = 0$ the $SU(2)_L \times SU(2)_R$ symmetry \eqref{eq:lsm:meson_symmetry} is exact,
      and we saw above that there is a degenerate range \TODO{JO: ``vacuum manifold''} of ground states.
...
\TODO{is this argument valid, with BEC etc? yes -- good as a motivator. mention $1/N_c$-scheme as a scheme in which everything is \emph{consistent} later!}
...
{\goldenstar Maximum mass star profile ($m_\sigma=\SI{800}{\mega\electronvolt}$, $B^\frac14 = \SI{27}{\mega\electronvolt}$, $P_c=10^{34.727} \, \si{\pascal}$)};
\end{tikzpicture}
