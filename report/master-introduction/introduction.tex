...
\begin{tikzpicture}[
	...
	quarkstar/.pic={
		...
		\draw[draw=black, fill=green!70!black, circular glow={fill=green!50!black}] circle [radius=45pt]; \node [element] at (90:0pt) {$u$, $d$, $s$};
		\draw[draw=none] (-0pt, -55pt) rectangle (+0pt, +55pt);
	},
	whitedwarf/.pic={
...
As first proposed by \cite{ref:quark_star_proposition_1,ref:quark_star_proposition_2},
...
Moreover, the observation of a pulsar with a sub-millisecond rotation period would strongly suggest the existence of strange quark stars. \cite{ref:glendenning} % sec 10.3 (PDF)
...
\begin{equation}
	Z(V, T, \mu_i) = \oint_- \pathintdif \bar{\psi} \oint_- \pathintdif \psi \oint_+ \pathintdif \phi \exp \bigg\{ \int_0^\beta \dif \tau \int_V \dif^3 x \, \Big( \lagr_E\big[\bar{\psi}, \psi, \phi\big] + \sum_i \mu_i j^0_i \Big) \bigg\} = e^{-\beta V \Omega} .
...
\footnote{%
	...
	They are named after their discoverers George Gamow and Mário Schenberg met at the famous Casino de Urca in Rio de Janeiro in Brazil, who
...
Finally, the Gell-Mann matrices $T^a$ are the $\smash{N_f^2} - 1$ generators of the symmetry group $SU(N_f)$
...
\end{equation}
By the chain rule and equation \eqref{eq:master_intro:densities} we can derive that the suitably named common quark chemical potential $\mu$ corresponds to the total quark density
\begin{equation}
	n = -\pdv{\Omega}{\mu} =
...
and we will later see that the additional constraints \eqref{eq:lsm:chemical_equilibrium} and \eqref{eq:lsm:charge_neutrality}
...
\item The \textbf{$\bm{N_c}$ approximation scheme} or \textbf{large $\bm{N_c}$-limit} is a scheme in which one makes expansions in the assumed small parameter $1/N_c$.
      Although there are only $N_c = 3$ colors in nature, this scheme has indeed been used to make accurate predictions. \cite{ref:large_Nc_review}
...
At low temperature and density, experiments and lattice simulations show that colored quarks never appear alone, but are always confined in hadrons that have no overall color.
...
The situation with low temperature and high baryon density is more uncertain:
