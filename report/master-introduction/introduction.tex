...
\begin{tikzpicture}[
	...
	\node (whitedwarf) at (0, 0) [label={above:\footnotesize$0.08 M_\odot \lesssim M \lesssim 8 M_\odot$}, label={below:\subcaption{\label{fig:master_intro:star_evolution_white_dwarf}white dwarf}}] {\tikz \pic {whitedwarf};};
	\node (neutronstar) at (3.3, 0) [label={above:\footnotesize$8 M_\odot \lesssim M \lesssim 40 M_\odot$}, label={below:\subcaption{\label{fig:master_intro:star_evolution_neutron_star}neutron star}}] {\tikz \pic {neutronstar};};
	\node (neutronstar) at (7.5, 0) [label={above:\footnotesize$M \approx 40 M_\odot$ (?)}, label={below:\subcaption{\label{fig:master_intro:star_evolution_quark_star}quark star (?)}}] {\tikz \pic {quarkstar};};
	\node (blackhole) at (11.5, 0) [label={above:\footnotesize$M \gtrsim 40 M_\odot$}, label={below:\subcaption{\label{fig:master_intro:star_evolution_black_hole}black hole}}] {\tikz \pic {blackhole};};
\end{tikzpicture}
\caption{\label{fig:master_intro:star_evolution_end}%
	The final stages of a star's life depending on its initial mass $M$.
...
is more or less predetermined by the initial mass $M$ of the embryonic \textbf{protostar} born from its parent \textbf{giant molecular cloud}.
Provided that $M \gtrsim 0.08 M_\odot$, the temperature in a protostar reaches the point necessary to ignite fusion of hydrogen, and the star becomes a \textbf{main-sequence star} -- otherwise it retires early as a \textbf{brown dwarf}.
If $0.08 M_\odot \lesssim M \lesssim 8 M_\odot$, a main-sequence star eventually explodes in a \textbf{planetary nebula} and leaves behind a core that collapses under gravity until it stabilizes as a \textbf{white dwarf} (\cref{fig:master_intro:star_evolution_white_dwarf}) supported by electron degeneracy pressure.
If $8 M_\odot \lesssim M \lesssim 40 M_\odot$, the main-sequence star rather explodes in a \textbf{supernova} and becomes a \textbf{neutron star} (\cref{fig:master_intro:star_evolution_neutron_star}) that is squeezed by gravity beyond the maximum supported pressure of a white dwarf, and reaches a new equilibrium mainly supported by degeneracy pressure of baryons and nuclei.
If $M \gtrsim 40 M_\odot$, the supernova remnant cannot support itself and collapses to a \textbf{black hole} (\cref{fig:master_intro:star_evolution_black_hole}).
\TODO{$M_0$ for init mass}
...
\begin{tabular}{ l r r l }
	...
	3C 58 & ??? & $\SI{15}{\kilo\meter}$ & \TODO{source} \\
...
As first proposed by Ivanenko and Kurdgelaidze \TODO{source}, however,
...
If the quark matter includes strange quarks in addition to up and down quarks, it is referred to as strange quark matter and the star is called a \textbf{strange quark star}.
...
Quarks and gluons that mediate the strong force between them are described by the theory of \textbf{quantum chromodynamics}.
Roughly speaking, it predicts that quarks are confined in hadrons at low densities due to a property called \textbf{color confinement},
while at extreme densities another property called \textbf{asymptotic freedom} implies that quark matter becomes deconfined and free of all interactions, forming \textbf{quark-gluon plasma}.
The hadron-confined and quark-gluon plasma phases are separated by the \textbf{chiral phase transition}.
Unfortunately, quantum chromodynamics is very hard to make analytical calculations with, so one must resort to approximations and alternative models.
...
\Cref{fig:master_intro:confinement} illustrates this process for a meson.
