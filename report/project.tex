...
\begin{equation}
	M = m(R) = \int_0^R \epsilon(r) 4 \pi r^2 \dif r .
...
\begin{equation*}
	\bar{M} = \int_0^R \epsilon(r) e^{\beta(r)} 4 \pi r^2 \dif r .
\end{equation*}
The difference
\begin{equation*}
	\bar{M} - M = \int_0^R \epsilon(r) \left( \left( 1-\frac{2Gm}{r} \right)^{-1/2} - 1 \right) 4 \pi r^2 \dif r > 0
\end{equation*}
...
\begin{equation}
	\bar{M} - M \approx \int_0^R \epsilon(r) \frac{GM}{r} 4 \pi r^2 \dif r .
\end{equation}
As shown in \cite{ref:mtw} (exercise 23.7), this is precisely the energy one gets when constructing the star by sequentially placing thin shells $4 \pi r^2 \dif r$ on top of each other, each subject to the gravitational attraction of the shells placed below it.
