...
To analyze astrophysical objects like stars, it is of considerable interest to relate the pressure $p(x)$ and energy density $\rho(x)$ at every position $x$ inside the object.
We will derive the relativistic relation between these quantities from the \textbf{Einstein equation} \cite{ref:carroll}
\begin{equation}
	G\indices{_\mu_\nu} = R_{\mu \nu} - \frac{1}{2} R g_{\mu \nu} = 8 \pi G T_{\mu \nu} ,
	\label{eq:einstein}
\end{equation}
...
\begin{subequations}
\begin{align}
	\frac{1}{r^2} e^{-2 \beta} \left( 2 r \partial_r \beta - 1 + e^{2 \beta} \right)  &= 8 \pi G \rho                                                                                && (G\indices{_t_t} = 8 \pi G T\indices{_t_t}) \label{eq:einstein_to_tov:tt} \\
	\frac{1}{r^2} e^{-2 \beta} \left( 2 r \partial_r \alpha + 1 - e^{2 \beta} \right) &= 8 \pi G p                                                                                   && (G\indices{_r_r} = 8 \pi G T\indices{_r_r}) \label{eq:einstein_to_tov:rr} \\
	e^{-2 \beta} \left( \partial_r^2 \alpha + (\partial_r \alpha)^2 - (\partial_r \alpha) (\partial_r \beta) + \frac{1}{r} (\partial_r \alpha - \partial_r \beta) \right) &= 8 \pi G p . && (G\indices{_\theta_\theta} = 8 \pi G T\indices{_\theta_\theta}) \label{eq:einstein_to_tov:thetatheta}
\end{align}
\end{subequations}
...
\end{equation}
directly relating $m(r)$ and $\rho(r)$.
If we set $m(0) = 0$, we can integrate to get
\begin{equation}
	m(r) = \int_0^r \rho(r) 4 \pi r^2 \dif r .
\end{equation}
Thus, $m(r)$ is simply the volume integral of the energy density $\rho(r)$ inside the radius $r$ and can be interpreted as the mass of that shell.
...
\end{equation}
To finally eliminate $\alpha$, we can replace all occurences of $\partial_r \alpha$ and $\beta$ in the remaining \cref{eq:einstein_to_tov:thetatheta} with the expressions \eqref{eq:einstein_to_tov:dadr1} and \eqref{eq:einstein_to_tov:def_m}.
...
An elegant, but more clever argument is to use local energy-momentum conservation $\nabla_\mu T\indices{^\mu^\nu} = 0$, which is both physically reasonable and in fact possible to prove directly from the Einstein equation \eqref{eq:einstein}.
...
\end{equation*}
Using $T\indices{^0_0} = -\rho$ and $T\indices{^1_1} = T\indices{^2_2} = T\indices{^3_3} = p$ from \cref{eq:einstein_to_tov:T}, the sums cancel, leaving
\begin{equation}
	\diff{\alpha}{r} = \frac{-1}{\rho+p} \diff{p}{r} .
...
\begin{equation}
	\diff{p}{r} = -\frac{(\rho+p) (G m(r) + 4 \pi G r^3 p)}{r (r - 2 G m(r))} .
	\label{eq:tov}
\end{equation}
It relates the pressure gradient $\diff{p}{r}$ and energy density $\rho$ at radius $r$ from the core of a spherical static star composed of a perfect fluid.
\Cref{eq:einstein_to_tov:m_rho,eq:tov} constitute two equations for the three unknowns $p$, $\rho$ and $m$.
To determine them, an additional equation of state $p = p(\rho)$ from the domain of thermodynamics and statistical physics is required.
...
\section{Deriving the Tolman-Oppenheimer-Volkoff equation from only the Einstein equation}
