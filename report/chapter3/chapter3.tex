...
Property \ref{item:nstars:sturm_liouville_completeness} implies that any sane displacement $\xi(t,r)$ can be written as the linear combiation
\begin{equation}
...
\TODO{innvending 1: vi gjør perturbasjonsteori, hvorfor er eksponensiell tidsvekst gyldig også når stjernen har blitt veldig mye større/mindre?}

\TODO{can the instability stuff be connect to attractor/chaos stuff from nonlinear dynamics?}
...
\end{itemize}
Our curve only includes a spiral turning counterclockwise and thus indicate the two first rules, but we have no grounds to confirm nor deny the third rule.
...
\addplot [mark=none, mesh, very thick] table [x expr={10*\thisrow{R}}, y=M, meta=nu] {../code/data/gr2.dat};
...
Do note the presence of all \emph{four} colors.
}
