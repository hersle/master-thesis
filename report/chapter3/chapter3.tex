...
\TODO{use small $u$ for velocity, so it does not conflict with big $U$ below}
\begin{subequations}
\begin{align}
...
\TODO{continue from here on friday}
...
\TODO{remark how we start with $U \propto r^3$, then see which eigenvalue that causes us to ``hit'' the correct boundary condition on the other side}
