...
Solving the Tolman-Oppenheimer-Volkoff equations \eqref{eq:tov:tovsys} yield $P_0$, $m_0$ and $\alpha_0$, from which one can calculate $\epsilon_0$ by the equation of state \eqref{eq:tov:eos} and $\beta_0$ from definition \eqref{eq:einstein_to_tov:def_m}.
...
Here, the number of unstable modes $n$ with $\omega_n^2 < 0$ for each star is displayed.
A star is stable only if it has $n = 0$ unstable modes.
\emph{Note the presence of all four colors.}
}
