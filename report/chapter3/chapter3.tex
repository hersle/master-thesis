...
At this point, let us remark that the neglection of the infinite contribution to the pressure \eqref{eq:nstars:pressure} is justified.
...
Having obtained equations of state $\epsilon = \epsilon(P)$, we are finally in a position to solve the Tolman-Oppenheimer-Volkoff equation \eqref{eq:tov}.
We will do so for all three equations of state that we encountered in \cref{sec:nstars:eos}.
...
By our definition, $\pdv{\xi}/{t} = \odv{r}/{t}$, where the right side is to be interpreted as the derivative of the fluid element's radial coordinate taken \emph{along its world line} -- or \emph{stream line} -- $x(\tau)$.
...
However, since the derivative $\odv{}/{\tau}$ is \emph{along the stream line} of the fluid element, it is more accurate to use a Lagrangian change over a Eulerian changes to first order in perturbation theory.
