...
\end{equation}
because the only spatial derivative that appears is of this precise combination in \TODO{ref eq}.
...
We can find a solution to this differential equation using separation of variables.
...
\begin{equation}
	...
	r = 0.
\end{equation}
From \TODO{ref e} and \TODO{ref gamma0}, we see that this is equivalent to
\begin{equation}
	\Delta P = 
...
\TODO{use $n(\omega^2)$ for number of nodes instead of distinguishing between $n$ and $N$?}
...
Finally, we apply the shooting method to every star along the curve in \cref{fig:nstars:massradius}.
...
Our computations suggest that at every local mass extremum point where $\odv{M(P_0)}/{P_0} = 0$, stars seem to acquire one more unstable vibration mode, starting with zero unstable modes for the lowest pressures $P_0 \rightarrow 0$.
In fact, \cite{ref:stability_methods} presents a method which is based on this principle!
For a sequence of \emph{cold} stars -- like ours -- in a mass-radius diagram like \cref{fig:nstars:massradius}, then the following holds:
\begin{itemize}
...
\TODO{can I really say this? I never drew a parametrical mass-radius curve for stars of constant radius}
