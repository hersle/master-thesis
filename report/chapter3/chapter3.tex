...
\pgfplotsinvokeforeach{0,...,31}{ % 31, it works but it is very slow
	%\pgfplotstablegetrowsof{\shoottable}
	%\pgfmathparse{int(\pgfplotsretval-1)}
	%\pgfplotstablegetelem{\pgfmathresult}{U#1}\of{\shoottable} % last value of U (positive -> blue, negative -> red)
	%\pgfmathparse{int(sign(\pgfplotsretval))}
	%\pgfmathsetmacro{\signU}{\pgfmathresult}
