...
\pgfplotsinvokeforeach{0,1,...,7} {
	...
	\pgfmathparse{sign(\pgfplotsretval)*sqrt(abs(\pgfplotsretval*3)) * 1e2)} % display in meaningful units, mult by c/r0
	\addlegendentryexpanded{$\omega_{#1}^2 = \pgfmathprintnumber[fixed, fixed zerofill, precision=2, showpos=true]{\pgfmathresult}^2 \, \si{\per\second\squared}$};
}
...
Eight lowest vibration modes $U_n(r)$ and squared eigenfrequencies $\omega_n^2$ for an unstable ideal neutron star with central pressure $P = \SI{1.28e37}{\pascal} \epsilon_0$.
