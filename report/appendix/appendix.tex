...
For more details, refer to detailed books or notes on the subject, like \cite{ref:carroll}, \cite{ref:mika_gr_notes} and \cite{ref:mtw}, all of whom this summary is based on.
...
They are rather intertwined into \emph{spacetime} -- a $(3+1)$-dimensional construct with coordinates given by a $4$-vector
\begin{equation}
	x^\mu = (\underbrace{x^0}_{\text{time}}, \underbrace{x^1, x^2, x^3}_{\text{space}}) .
\end{equation}
In flat Euclidean space, the coordinates could be taken as $x^\mu = (ct, x, y, z)$.
Mathematically, the geometry of spacetime is described by a \emph{Riemannian manifold} that generalizes flat Euclidean space to \emph{curved space}.
At every point in such a manifold, spacetime locally resembles Euclidean space in the \emph{tangent space} located at that point, and all such tangent spaces vary in a smooth manner from point to point.
For example, \cref{fig:tangent_space} pictures the tangent space at a point of the $2$-sphere manifold.
Concepts like angles, lengths, area and volume apply locally at each Euclidean tangent space, and one can generalize such concepts to the full manifold by integrating the local contributions from one point in the manifold to another.

\begin{figure}
\centering
\includesvg[width=0.60\textwidth]{figures/tangent-space.svg}
\caption{\label{fig:tangent_space}The tangent space at a point $x$ on the $2$-sphere manifold can be pictured as the tangent plane at that point. If a vector field $V^\mu(x^\nu)$ is placed on the manifold, the vector would lie in this tangent space.}
% link: https://commons.wikimedia.org/wiki/File:Image_Tangent-plane.svg
\end{figure}
...
\end{equation}
One can verify that the set of all differential operators, implicitly assumed to work on some scalar function, satisfy all criterions for being a vector space.
...
\end{equation}
as a vectorial object with components $\odv{x^{\mu}}/{\lambda}$ and basis vectors $\partial_\mu$.
Under a change of coordinates $x^{\mu'} = x^{\mu'}(x^\mu)$, the directional derivative transforms like
\begin{equation}
	\odv{}{\lambda} = \odv{x^{\mu'}}{\lambda} \partial_{\mu'} = \underbrace{\Bigg( \odv{x^{\mu}}{\lambda} \pdvx{\mu'}{\mu} \Bigg)}_\text{components} \underbrace{\Bigg( \pdvx{\mu}{\mu'} \partial_{\mu} \Bigg)}_\text{basis} = \odv{x^{\mu}}{\lambda} \partial_{\mu} .
\end{equation}
We define a \textbf{covariant vector} as a tuple that transforms \emph{with} the change of basis, like
\begin{equation}
	V_{\mu'} = \pdv{x^{\mu}}{x^{\mu'}} V_\mu .
	\label{eq:covariant_transformation}
\end{equation}
Oppositely, a \textbf{contravariant vector} is a tuple that transforms \emph{opposite to} the basis, like
\begin{equation}
	V^{\mu'} = \pdv{x^{\mu'}}{x^\mu} V_\mu .
	\label{eq:contravariant_transformation}
\end{equation}
More generally, we define a \textbf{tensor} of rank $(m,n)$ as an array composed of $m$ contravariant indices and $n$ covariant indices that transforms like
\begin{equation}
...
\end{equation}
contains inner products between basis vectors $\vec{e}_\mu$ that span the tangent spaces at each point $x$ in the manifold.
...
Similarly, we could use the inverse metric $g^{\mu \nu}$ to raise indices.
Any rank-$2$ tensor would suffice to raise and lower indices, but the metric is a natural choice because it is inherent to the manifold.
...
\end{equation} 
that extends the concept of distance locally to every point in the manifold.
By integrating the line element from one point in the manifold to another, one can compute the total distance
\begin{equation}
...
The factor $\sqrt{\det{\gamma}}$ arises to make the volume element $\dif^n x \sqrt{\det{\gamma}}$ invariant between coordinate systems.
...
\subsection{Parallell transport and geodesic equation}

\begin{figure}
\centering
\includesvg[width=0.5\textwidth]{figures/parallel_transport.svg}
\caption{\label{fig:parallel_transport}When a vector is parallel transpored around a closed loop on the $2$-sphere, its final direction depends on the path taken. Illustration by Fred the Oyster on Wikimedia Commons (\url{https://commons.wikimedia.org/wiki/File:Parallel_Transport.svg}), licensed under \href{https://creativecommons.org/licenses/by-sa/4.0/}{CC BY-SA 4.0}}
\end{figure}
...
Instead, gravity presents itself in the geometry of spacetime itself, as the presence of energy-momentum curves spacetime and lays geodesic ``tracks'' according to \cref{eq:geodesic} that any freely falling particle is destined to follow to whichever fate awaits at the end of the line.
...
\end{equation}
The prefactor has been conventiently chosen yield correct result \eqref{eq:einstein_derivation:einstein_matter} in the end, which we saw in \cref{sec:einstein_to_poisson} led to Newtonian gravity in the Newtonian limit.
