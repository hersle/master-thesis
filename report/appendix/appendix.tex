...
\begin{equation}
\begin{split}
	f(\psi) &= a + \sum_i a_i \psi_i + \sum_i b_i \psi^\dagger_i + \sum_{i,j} a_{ij} \psi_i \psi_j + \sum_{i,j} b_{ij} \psi_i \psi^\dagger_j + \sum_{i,j} c_{i,j} \psi^\dagger_i \psi^\dagger_j \\
	        &+ \ldots + d \psi^\dagger_1 \psi_1 \cdots \psi^\dagger_N \psi_N
\end{split}
\end{equation}
...
Also note that the product rule
\begin{equation}
\begin{split}
...
\end{split}
\end{equation}
works out as we are used to with complex numbers.
\TODO{wrong!}
