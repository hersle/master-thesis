...
we encountered ($N_c$ copies of) the ultraviolet-divergent Dirac sea vacuum contribution % ref:lsm3f calls it this
\begin{equation}
...
\end{split}
\end{equation}
We now cancel $\Gamma\big(\frac{d}{2}\big)$ and replace $\Gamma\big(\textstyle{-\frac12}\big) = -2\sqrt{\pi}$.
...
In this appendix, we explain and lay out all Python code used to produce the results of this thesis.
...
These programs find all equations of state and stellar solutions in chapters \ref{chap:mit}, \ref{chap:lsm2f}, \ref{chap:lsm3f} and \ref{chap:hybrid}.
The models are organized as classes according to the following diagram:
...
\begin{tikzpicture}[
	...
	level 2/.style={sibling distance=1.5cm, anchor=west},
]
\node {Model}
...
\subsection*{Common code for all quark and hybrid star models (\cref{chap:mit,chap:lsm2f,chap:lsm3f,chap:hybrid})}
