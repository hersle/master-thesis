...
When deriving \cref{eq:tov} analytically, we made use of energy-momentum conservation $\nabla_\mu T\indices{^\mu^\nu} = 0$ instead of making use of \cref{eq:einstein_to_tov:thetatheta}.
Inspired by \cite{ref:sage_tov}, we have here written a small program that does the latter in the computer algebra system SAGE.

\codefile{python}{../code/tov_derive.sage}{tov\_derive.sage}

The output of the program matches \cref{eq:tov} precisely.
...
With the dimensionless variables \eqref{eq:numtov:dimensionless_variables}, the Tolman-Oppenheimer-Volkoff system \eqref{eq:numtov:tovsys} becomes
\begin{subequations}
...
\codefile{python}{../code/tov_solve.py}{tov\_solve.py}
...
\codefile{python}{../code/tov_free_fermi_gas.py}{tov\_free\_fermi\_gas.py}
