...
Functions of Grassmann numbers are \emph{defined} by their Taylor expansion.
...
\begin{split}
	f(\psi) &= A + \sum_i A_i \psi_i + \sum_i B_i \conj\psi_i + \sum_{i,j} a_{ij} \psi_i \psi_j + \sum_{i,j} B_{ij} \psi_i \conj\psi_j + \sum_{i,j} C_{i,j} \conj\psi_i \conj\psi_j \\
	        &+ \ldots + Z_{1 \cdots N} (\psi_1 \cdots \psi_N) (\conj\psi_1 \cdots \conj\psi_N)
\end{split}
...
Note that if one integrates the function \eqref{eq:gnums:function_taylor_series} over all Grassmann numbers and their conjugates, the only term that survives
\begin{equation}
	\int \dif^N \conj\psi \int \dif^N \psi \, (\psi_1 \cdots \psi_N) (\conj\psi_1 \cdots \conj\psi_N) = 1 .
\end{equation}
...
Second, recall that the \textbf{residue of a quotient function} $f(z) / g(z)$ at a simple pole $z = z_0$ where $g(z_0) = 0$, but $g'(z_0) \neq 0$ is 
