...
\fi
\def\N{8}
\def\smallr{0.25}  % TODO: make them touch?\\
\def\drawpoles{
\foreach \n in {1,2,...,\N} {
	\def\y{-\r + (\n-1)/(\N-1)*(2*\r)}
	\node[draw,cross] at (0,{\y}) {};
	%\draw[thick, red, decoration={markings, mark=at position 0.125 with {\arrow{>}}}, postaction={decorate}] (0,{\y}) circle [radius=\smallr];
}
\node[label=right:{\scriptsize $iE_n$}] at (0,\r/2) {};
\node[draw,cross,label=above:{\scriptsize $+E$}] at (+\r/2,0) {};
\node[draw,cross,label=above:{\scriptsize $-E$}] at (-\r/2,0) {};
}
\def\drawaxes{
	\draw[->, black!50!white, thin] (-1.32*\r, 0) -- (+1.32*\r, 0.0) node [above, black] {\scriptsize $\text{Re}(z)$};
	\draw[->, black!50!white, thin] (0, -1.32*\r) -- (0, +1.32*\r) node [right, black] {\scriptsize $\text{Im}(z)$};
}
\def\cross{
...
\drawpoles
\iffalse
\draw[thick, red, decoration={markings, mark=at position 0.125 with {\arrowreversed{>}}}, postaction={decorate}] (0, 0) circle [radius=(\r+\smallr)];
...
\foreach \n in {1,2,...,\N} {
	\def\y{-\r + (\n-1)/(\N-1)*(2*\r)}
	\draw[thick, red!50!white, decoration={markings, mark=at position 0.125 with {\arrow{>}}}, postaction={decorate}] (0,{\y}) circle [radius=\smallr];
}
\draw[very thick, red, decoration={
	markings,
	mark=at position 0.125 with {\arrowreversed{latex}},
}, postaction={decorate}] (0, 0) circle [radius=\r+\smallr/2];
\end{tikzpicture}
...
\draw[
	...
	very thick, red
] (+\w/2, -\r) -- (+\w/2, +\r) arc [start angle=+90, end angle=-90, radius=\r];
\draw [
	decoration={
		markings, 
	...
	very thick, red
] (-\w/2, +\r) -- (-\w/2, -\r) arc [start angle=270, end angle=90, radius=\r];
\end{tikzpicture}
...
\caption{\label{fig:matsum:contours}%
	...
	\TODO{update text and figure}
}
\end{figure}

For reasons that will soon be clear, consider the contour integral
\begin{equation}
	\sum_{n=-N}^N \oint_{C_n} \dif z \, s(-iz) n_\pm(z)
\label{eq:matsum:contour_integral}
\end{equation}
along the contours $C_n$ drawn in \cref{fig:matsum:contourA} that surround the $2N+1$ poles of $n_\pm(z)$ that are closest to the origin.
To proceed in a mathematically well-defined way, we consider only a finite number of poles throughout the derivation, but will take the limit $N \rightarrow \infty$ at the very end to include all poles.
...
\begin{equation}
	\sum_{n=-N}^N \oint_{C_n} \dif z \, s(-iz) n_\pm(z) = 2 \pi i \sum_i \res_{z = i E_n}[s(-i z) n_\pm(z)]
	                                  = \pm \frac{2 \pi i}{\beta} \sum_{n=-N}^N s(E_n) .
\label{eq:matsum:connection_integral_sum}
\end{equation}
In other words, the finite version of the Matsubara sum \eqref{eq:matsum:sum} is given by the contour integral
\begin{equation}
	S_N = \sum_{n=-N}^N s(E_n) = \pm \frac{\beta}{2 \pi i} \oint_{C_n} \dif z \, s(-iz) n_\pm(z) ,
\label{eq:matsum:sum_as_contour_integral}
\end{equation}
and the full sum is given by $S_\infty = \lim_{N \rightarrow \infty} S_N$.

What is the use of transforming a simple sum into a complex (in both senses) contour integral?
Consider now the circular contour $C_B$ in \cref{fig:matsum:contourB} that touches $C_{-N}$ and $C_N$ at the intersections with the imaginary axis.
Now
\begin{equation}
	\sum_{n=-N}^N \oint_{C_n} \dif z +
	\oint_{C_B} \dif z =
	\oint_{C_C} \dif z ,
\label{eq:matsum:contour_superposition}
\end{equation}
where $C_C$ is the contour shown in \cref{fig:matsum:contourB}.
The overlapping contributions between the contours $C_n$ run opposite ways and cancel, producing the vertical parts of $C_C$.
The clockwise circular contour $C_B$ cancels the top part of $C_{N}$ and the bottom part of $C_{-N}$ and closes the contour with semicircles in the real half planes.

Finally, include all poles by taking the limit $N \rightarrow \infty$.
This inflates the contour $C_C$ to the full real half planes, except the imaginary axis $\real Z = 0$.
Assuming $s(-i z)$ -- as in our motivating example \eqref{eq:matsum:motivating_example} -- satisfies
\begin{equation}
	\abs{s(-iz) n_\pm(z)} < \frac{1}{\abs{z}} \quad \text{as} \quad \abs{z} \rightarrow \infty ,
\end{equation}
then $\oint_{C_B} \dif z \, s(-iz) n_\pm(z) \rightarrow 0$ as $N \rightarrow \infty$, and by \cref{eq:matsum:contour_superposition} we can trade the contour $\sum_n C_n$ for $C_C$ in \cref{eq:matsum:connection_integral_sum}.
...
\TODO{control the revised argument}
