...
As a simple example and to make sense of some of our results, let us consider the $2$-sphere of constant radius $r$ with coordinates
\begin{equation}
	(x^1, x^2) = (\theta, \phi)
	\quad \text{with} \quad
	0 \leq \theta < \pi , \quad
...
For example, parallel transporting a vector with components $(V^1, V^2) = (V^\theta, V^\phi)$ along a curve with $\odv{\theta}/{\tau} = \dot\theta \neq 0$, but $\odv{\phi}/{\tau} = 0$ turns the equation of parallel transport \eqref{eq:parallel_transport} into
\begin{equation}
	\dot{V}^\theta = 0 , \qquad
	\dot{V}^\phi + \frac{1}{\tan \theta} \dot\theta V^\phi = 0 .
\end{equation}
The former equation yields $V^1 = \text{const}$, and one can therefore differentiate the latter with the chain rule, exploiting $\pdv{V^2}/{\theta} = 0$, to obtain
\begin{equation}
	\odv{V^1}{\theta} = 0 , \qquad
	\pdv{V^2}{\phi} + \frac{1}{\tan \theta} V^2 = 0 .
\end{equation}
The solution is
\begin{equation}
	V^\theta(\theta, \phi) = V^\theta(\theta_0, \phi_0) , \qquad
	V^\phi(\theta, \phi) = \frac{V^\phi(\theta_0, \phi_0)}{\sin \theta} .
\end{equation}
As predicted \TODO{ref?}, the length is preserved
\begin{equation}
	V^\mu V_\mu = 
	g_{11} (V^1)^2 + g_{22} (V^2)^2 =
	R^2 (V^1_0)^2 + R^2 (V^2_0)^2 = \text{const} .
\end{equation}

\TODO{geodesic equation?}

The nonzero components of the Riemann curvature tensor \eqref{eq:def_riemann_tensor} becomes
\begin{equation}
	...
	R\indices{^2_1_2_1} = 1 .
\end{equation}
Then the nonzero components of the Ricci tensor \eqref{eq:def_ricci_tensor}
\begin{equation}
	R_{00} = 1 , \qquad R_{11} = \sin^2 \theta .
\end{equation}
Finally, the Ricci curvature scalar \eqref{eq:def_ricci_scalar} is
\begin{equation}
	R = \frac{2}{R^2} .
\end{equation}
As we should expect from dimensional analysis of the preceeding equations, it is proportional to $1/R^2$, where $R$ is the only length scale defined in the problem.
