...
Under a change of coordinates $x^{\mu'} = x^{\mu'}(x^\mu)$, the directional derivative transforms as
\begin{equation}
...
\end{equation}
under a coordinate transformation $x^{\mu'} = x^{\mu'} (x^\mu)$.
...
(TODO: explain that there are no ``physics'' (?) involved in these criteria -- the only ``job'' of the connection is to specify the covariant derivative)
\iffalse
\begin{align}
