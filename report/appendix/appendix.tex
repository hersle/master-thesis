...
Let us derive the rate of change of this volume element in flat spacetime, then generalize the result using the equivalence principle \TODO{ref}.
...
\TODO{remark second equality due to vanishing Christoffel symbols?}
\begin{equation}
...
In contrast \TODO{to what?}, the total \textbf{baryon number} $N = n V$ must be conserved, as expressed by
\begin{equation}
...
\TODO{show that this is a continuity equation?}
...
\TODO{show projector property $P^2 = P$!}
...
\TODO{what about Newt limit by $c \rightarrow \infty$?}
...
Euler \eqref{eq:relfluid:euler_equation} with $P = \text{const}$, $\epsilon = \text{const}$, flat space (Minkowski metric).
Apply perturbation on top, then last term is small and we get.
\begin{equation}
	\pdv{\vec{v}}{t} = - \frac{c^2 \nabla (\delta P)}{\epsilon + P} .
\end{equation}
Adiabatic condition \eqref{eq:relfluid:energy_conservation_rewritten} gives
\begin{equation}
	\delta \epsilon = \left( \epsilon + P \right) \frac{\delta n}{n} .
\end{equation}
Combining the two gives
\begin{equation}
	\pdv{\vec{v}}{t} = -c^2 \frac{\nabla P}{\delta \epsilon} \frac{\delta n}{n}
	                 = -c^2 \frac{\delta P}{\delta \epsilon} \frac{\nabla (\delta n)}{n}
	                 = -v_s^2 \frac{\nabla (\delta n)}{n}
\end{equation}
Combine this with first-order version of \eqref{eq:relfluid:baryon_number_rate_change},
\begin{equation}
	\odv{\delta n}{t} + n \nabla \cdot \vec{v} = 0,
\end{equation}
to find the wave equation
\begin{equation}
	\left( \odv[2]{}{t} - v_s^2 \nabla^2 \right) \delta n = 0
\end{equation}
showing that $v_s$ is indeed the velocity of sound waves.
...
\formulawithproof{Degenerate Fermi gas energy density integral}{
	...
	\begin{aligned}
		I &= \int_0^{x_F} \dif x \, x^2 \sqrt{x^2 + 1} \\
		  &= \int_0^{\theta_F} \dif \theta \, \cosh^2 \theta \sinh^2 \theta               & \quad & \Big( \text{use } \cosh^2 \theta - \sinh^2 \theta = 1 \Big) \\
		  &= \frac14 \int_0^{\theta_F} \dif \theta \, \sinh^2 (2 \theta)                  & \quad & \Big( \text{use } \sinh (2 \theta) = 2 \sinh \theta \cosh \theta \Big) \\
		  &= \frac18 \int_0^{\theta_F} \dif \theta \, \left[ \cosh (4 \theta) - 1 \right] & \quad & \Big( \text{use } \sinh^2 \theta = \frac12 \left[ \cosh (2 \theta) - 1 \right] \Big) \\
		  &= \frac{1}{8} \left[ \frac14 \sinh (4 \asinh x_F) - \asinh (x_F) \right] . \\
	\end{aligned}
	\end{equation*}
...
\formulawithproof{Degenerate Fermi gas pressure integral}{
	...
	\begin{aligned}
		I &= \int_0^{x_F} \frac{\dif x \, x^4}{\sqrt{x^2 + 1}} \\
		  &= \int_0^{\theta_F} \dif \theta \, \sinh^4 \theta & & \Big( \text{use } \cosh^2 \theta - \sinh^2 \theta = 1 \Big) \\
		  %&= \frac14 \int_0^{\theta_F} \dif \theta \, \left[ \cosh (2 \theta) - 1 \right]^2  & & \Big( \text{use } \sinh^2 \theta = \frac{1}{2} \left[ \cosh ( 2 \theta) - 1 \right] \Big) \\
		  &= \frac14 \int_0^{\theta_F} \dif \theta \, \left[ \cosh^2 (2 \theta) - 2 \cosh (2 \theta) + 1 \right] & & \Big( \text{use } \sinh^2 \theta = \frac{1}{2} \left[ \cosh ( 2 \theta) - 1 \right] \Big) \\
		  &= \frac18 \int_0^{\theta_F} \dif \theta \, \left[ \cosh (4 \theta) - 4 \cosh (2 \theta) + 3 \right] & & \Big( \text{use } \cosh^2 \theta = \frac12 \left[ \cosh (2 \theta) + 1 \right] \Big) \\
		  &= \frac{1}{8} \left[ \frac14 \sinh (4 \asinh x_F) - 2 \sinh (2 \asinh x_F) + 3 \asinh (x_F) \right] . \\
	\end{aligned}
	\end{equation*}
	Inserting the definitions
	\begin{equation*}
...
\TODO{right-align right parentheses everywhere, fix overfull hbox in equation above}
...
\TODO{fix code files so all lines are within margins}
