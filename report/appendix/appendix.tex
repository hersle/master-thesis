...
\begin{equation}
	S = \sum_{n=-\infty}^{+\infty} s(\omega_n)
\label{eq:matsum:sum}
\end{equation}
over all \textbf{Matsubara frequencies}
\begin{equation}
	\omega_n = \begin{dcases}
	               2 \pi n / \beta \hbar           & \text{for bosons}    \\
				   2 \pi \left( n+\frac12 \right) / \beta \hbar & \text{for fermions} .\\
			   \end{dcases}
\end{equation}
For example, we encountered the sum
\begin{equation}
	S = \sum_{n=-\infty}^{+\infty} \frac{1}{\omega_n^2 + \omega^2}
...
These are the familiar Bose-Einstein distribution $n_+(z)$ for bosons and Fermi-Dirac distribution $n_-(z)$ for fermions.
Importantly, they have simple poles at all imaginary Matsubara frequencies $z = i \omega_n$.
...
\def\n{16}
\def\drawpoles{
	\path[blue, decoration={markings, mark=between positions 0.01 and 1 step 3mm with {\cross}}, postaction={decorate}] (0, -\r+\w/2) -- (0, +\r-\w/2) node [black, right=0.1cm, yshift=-0.5cm] {\scriptsize $i \omega_n$};
	\path[blue, decoration={markings, mark=between positions 0 and 1 step 1.0 with {\cross}}, postaction={decorate}] (-\r/2, 0) node[black, above] {\scriptsize $-\omega$} -- (+\r/2, 0) node [black, above] {\scriptsize $+\omega$};
}
\def\drawaxes{
...
\end{equation}
along the contour in \cref{fig:matsum:contourA} that encloses the poles of $n_\pm(z)$ and is closed at $z = i \infty$ and $z = -i \infty$.
...
(TODO: write residues as $\res_{z=z_n} f(z)$, but with $z=z_n$ \emph{below} the $\res$ symbol?, see operatorname: \url{https://en.wikibooks.org/wiki/LaTeX/Advanced_Mathematics#Custom_operators}, also do this for other operators!)
\begin{equation}
...
\end{equation}
Let us apply this with $f(z) = s(-iz)$ and $g(z) = n_\pm(z)^{-1} = e^{\beta z} \mp 1$,.
We then find that the residues of the integrand in the contour integral \eqref{eq:matsum:contour_integral} are
\begin{equation}
	\res[s(-iz_n) n_\pm(z_n)]_{z_n = i \omega_n} = \pm \frac{1}{\beta} s(\omega_n) ,
\end{equation}
By the residue theorem \eqref{eq:matsum:residue_theorem}, the contour integral \eqref{eq:matsum:contour_integral} is then
\begin{equation}
	\oint_{C_A} \dif z \, s(-iz) n_\pm(z) = 2 \pi i \sum_i \res[s(-i z_n) n_\pm(z_n)]_{z_n = i \omega_n}
	                                  = \pm \frac{2 \pi i}{\beta} \sum_n s(\omega_n)
...
\end{equation}
where we have weighted the sum function $s(-iz)$ by the auxiliary function $n_\pm(z)$.
...
and integrating along $C_A$ in the contour integral \eqref{eq:matsum:contour_integral} is the same as integrating along $C_A$ \emph{and} $C_B$.
But superposing the contour $C_B$ on $C_A$ is equivalent to opening the closures at $z = i \infty$ and $z = -i \infty$ and closing the contour with half circles in the real half planes, resulting in the contour $C_C$ in \cref{fig:matsum:contourC}.
...
\begin{equation}
	s(-iz) = \frac{1}{-z^2 + \omega^2} .
\end{equation}
It has poles at $z = \omega$ and $z = -\omega$.
Using \cref{eq:matsum:residue_quotient} with $f(z) = n_\pm(z)$ and $g(z) = f(-iz)^{-1}$, we obtain the residues
\begin{equation}
	\res[s(-iz) n_\pm(z)]_{z =  \omega} = -\frac{1}{2 \omega} n_\pm(+\omega) 
	\qquad \text{and} \qquad
	\res[s(-iz) n_\pm(z)]_{z = -\omega} =  \frac{1}{2 \omega} n_\pm( \omega) .
\end{equation}
...
\begin{equation}
	S = \sum_{n=-\infty}^{+\infty} \frac{1}{\omega_n^2 + \omega^2}
	  ...
	  = \mp \frac{\beta}{2 \omega} \left( n_\pm(-\omega) - n_\pm(\omega) \right) .
\end{equation}
...
\begin{equation}
	S = \frac{\beta}{2 \omega} \left( 1 \pm \frac{2}{e^{\beta \omega} \mp 1} \right)
	  = \frac{\beta}{2 \omega} \left( 1 \pm 2 n_\pm(\omega) \right) .
\end{equation}
