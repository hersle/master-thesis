...
In addition, we couple the conserved vector current $\smash{j_f^\mu} = \bar{q}_f \gamma^\mu q_f$ to a chemical potential $\mu_f$ for each quark flavor,
...
Iron-56 is the most stable nuclide with an energy of $E/N_B = \epsilon/n_B = \SI{930}{\mega\electronvolt}$ per baryon.
...
For the two-flavor and three-flavor equations of state in \cref{fig:mit:eos}, the program reports the bag window
\begin{equation}
...
\nextgroupplot[
	...
	title = {Two-flavor MIT bag model quark stars\\$B^\frac14 = \{145,150,155\}\,\si{\mega\electronvolt}$}, title style={text width=7.5cm},
];
...
\nextgroupplot[
	xlabel={$R \, / \, \si{\kilo\meter}$},
	yticklabels={,,},
	title = {Three-flavor MIT bag model quark stars\\$B^\frac14 = \{145,150,155\}\,\si{\mega\electronvolt}$}, title style={text width=7.5cm},
...
modified by the bag shift \eqref{eq:mit:bag_shift} with three bag constants $B$ in the bag window \eqref{eq:mit:bag_window}.
...
\item The smaller the bag constant, the larger and more massive the star.
      ...
      Going forward, we therefore pay most attention to the lowest $B$.
...
which in turn can decay to three-flavor quark matter if the strange matter hypothesis is true.
