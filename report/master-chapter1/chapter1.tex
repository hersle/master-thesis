...
As we have simply forgot the gluons, we will continue to use the lone quark masses in \cref{tab:qcd:quark_properties}.
...
Adding a background of free electrons and reinstating the Fermi momenta $p_f = \sqrt{\smash[b]{\mu_f^2-m_f^2}}$ for each particle,
...
\end{equation}
Note that this expression gains an imaginary part when $\mu_i < m_i$, but we always mean to take the real part of it, or equivalently consider it to be multiplied by step functions $\Theta(\mu_i-m_i)$.
The rigorous understanding of this can be traced back to the zero-temperature calculation of the pressure \eqref{eq:nstars:pressure_zeroT}, in which the integrand contained a step function $\Theta(\mu-E(p)) = \Theta(\mu-\sqrt{\smash[b]{m^2+p^2}})$ and we took for granted that $\mu > m$.
...
\end{equation}
and $\mu_u$, $\mu_d$ and $\mu_s$ then follow from definition \eqref{eq:master_intro:chemical_potentials_transformed} and the chemical equilibrium constraint \eqref{eq:lsm:chemical_equilibrium}. 
...
it is instructive and in fact not far from accurate to solve this problem analytically in the ultra-relativistic limit $\mu_i \gg m_i$ where we neglect all masses.
...
We now calculate the general solution with massive quarks using the program in \TODO{ref appendix},
...
\TODO{emphasize/explain zero pressure?}
\begin{equation}
...
\TODO{possible to solve TOV analytically with $\epsilon = 3P+4B$ and $B \neq 0$? power series in $P(r) = \sum_n A_n r^n$?}
