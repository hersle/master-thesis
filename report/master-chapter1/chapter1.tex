...
\TODO{confusing, why not just $\mu_u$?}
\begin{equation}
...
\item Using bag constants inside the bag window \eqref{eq:mit:bag_window},
      the MIT bag model realizes quark stars with maximum masses $1.6 \, M_\odot \leq M \leq 1.7 \, M_\odot$ with $N_f=2$ flavors
      and $1.7 \, M_\odot \leq M \leq 2.0 \, M_\odot$ with $N_f=3$ flavors,
...
We obtained maximum masses $1.6 \, M_\odot \leq M \leq 1.7 M_\odot$ with $N_f=2$ flavors
and $1.7 \, M_\odot \leq M \leq 2.0 \, M_\odot$ with $N_f=3$ flavors.
This concludes our introduction to the basic concepts to be used in the remainder of this thesis.

\TODO{as in \cite{ref:quark_star_review}, $B$ is diff between the perturbative (chirally symmetric) and the non-perturbative (symmetry-broken) vacua, NOT confined/deconfined?}

\TODO{possible to solve TOV analytically with $\epsilon = 3P+4B$ and $B \neq 0$? power series in $P(r) = \sum_n A_n r^n$?}

\TODO{why zero pressure in stability criterion??? JO nor I understand properly. found somewhere saying $P=0$ corresponds to stability of the system, therefore comparison is done at $P=0$.}

\TODO{confinement/deconfinement: inspiration from Kyllingstad figure 1.5?}

\TODO{mention Polyakov loop as order parameter of confinement? see e.g. \url{https://crunch.ikp.physik.tu-darmstadt.de/hirschegg/2010/talks/Thu/Schaefer.pdf} there is e.g. Polyakov-loop extended linear sigma model}
