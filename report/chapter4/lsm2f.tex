...
\addplot+ [solid,          mesh, x filter/.expression={\thisrow{P} < 0.00139 ? x : nan}] table [x=R, y=M, meta expr={log10(\thisrow{P}*e0)}] {../code/data/quarkstar2f_B14_27.dat} node [Bpin, pos=0.918] {$B = (\SI{27}{\mega\electronvolt})^4$};
...
\addplot+ [solid,          mesh, x filter/.expression={\thisrow{P} < 0.0015 ? x : nan}] table [x=R, y=M, meta expr={log10(\thisrow{P}*e0)}] {../code/data/quarkstar2f_B14_34.dat} node [Bpin, pos=0.915] {$B = (\SI{34}{\mega\electronvolt})^4$};
...
\addplot+ [solid,          mesh, x filter/.expression={\thisrow{P} < 0.0013 ? x : nan}] table [x=R, y=M, meta expr={log10(\thisrow{P}*e0)}] {../code/data/quarkstar2f_B14_41.dat} node [Bpin, pos=0.915] {$B = (\SI{41}{\mega\electronvolt})^4$};
...
\addplot+ [solid,          mesh, x filter/.expression={\thisrow{P} < 0.0016 ? x : nan}] table [x=R, y=M, meta expr={log10(\thisrow{P}*e0)}] {../code/data/quarkstar2f_B14_48.dat} node [Bpin, pos=0.88] {$B = (\SI{48}{\mega\electronvolt})^4$};
...
What we have done is inconsistent because we have used a one-loop grand potential (in quarks), but matched its parameters using tree-level relations.
This has been done consistently to one-loop by \TODO{ref JO}, finding the effective potential
\TODO{disctinction between $\Delta$ and $g \avg{\sigma}$ and $g_R \avg{\sigma}$?}
\begin{equation}
\begin{split}
	\Omega &= \frac12 m_R^2 \avg{\sigma}^2 + \frac{\lambda_R}{24} \avg{\sigma}^4 - h_R \avg{\sigma} + \frac{N_c N_f}{16 \pi^2} g_R^4 \avg{\sigma}^4 \bigg[ \frac32 + \log \frac{\Lambda^2}{g_R^2 \avg{\sigma}^2} \bigg] + \text{(matter)}
\end{split}
\end{equation}
with running couplings
\begin{align}
	m_R^2 &= \frac{m_0^2}{1 - \frac{4 g_0^2 N_c}{(4\pi)^2} \log \frac{\Lambda^2}{\Lambda_0^2}}, \\
	g_R &= \frac{g_0}{\sqrt{1 - \frac{4 g_0^2 N_c}{(4\pi)^2} \log \frac{\Lambda^2}{\Lambda_0^2}}}, \\
	\lambda_R &= \frac{\lambda_0 - \frac{48 g_0^4 N_c}{(4\pi)^2} \log \frac{\Lambda^2}{\Lambda_0^2} }{\Big(1 - \frac{4 g_0^2 N_c}{(4\pi)^2} \log \frac{\Lambda^2}{\Lambda_0^2}\Big)^2}, \\
	h_R &= \frac{h_0}{1 - \frac{2 g_0^2 N_c}{(4\pi)^2} \log \frac{\Lambda^2}{\Lambda_0^2}} .
\end{align}
Insert into potential above and find
\begin{equation}
\begin{split}
	\Omega_{1L} &= \frac{3 m_\pi^2}{4} \Bigg\{ 1 - \frac{4 m_q^2 N_c}{(4\pi)^2 f_\pi^2} m_\pi^2 F'(m_\pi^2) \Bigg\} \avg{\sigma}^2 \\
...
\end{split}
\end{equation}
Independent of $\Lambda$, hallmark of consistent renormalization.
...
\addplot+ [orange, solid, opacity=0.5, forget plot] table [x=mu, y expr={g*\thisrow{sigma}}] {../code/data/2flaveos.dat};
\addplot+ [red, solid, opacity=0.5, forget plot] table [x=mu, y=muu] {../code/data/2flaveos.dat};
\addplot+ [darkgreen, solid, opacity=0.5, forget plot] table [x=mu, y=mud] {../code/data/2flaveos.dat};
\addplot+ [blue, solid, opacity=0.5, forget plot] table [x=mu, y=mue] {../code/data/2flaveos.dat};

\addplot+ [orange, solid] table [x=mu, y=Delta] {../code/data/2flavrefeos.dat}; \addlegendentry{$\Delta \, / \, \si{\mega\electronvolt}$};
\addplot+ [red, solid] table [x=mu, y=muu] {../code/data/2flavrefeos.dat}; \addlegendentry{$\mu_u \, / \, \si{\mega\electronvolt}$};
\addplot+ [darkgreen, solid] table [x=mu, y=mud] {../code/data/2flavrefeos.dat}; \addlegendentry{$\mu_d \, / \, \si{\mega\electronvolt}$};
\addplot+ [blue, solid] table [x=mu, y=mue] {../code/data/2flavrefeos.dat}; \addlegendentry{$\mu_e \, / \, \si{\mega\electronvolt}$};
...
\draw [draw=none, fill=gray!50] (250, -0.005) rectangle (350, 0.02);
\draw [-Latex, gray] (300,0.03) to [out=90, in=0] (232, 0.75);
\addplot+ [red, solid, opacity=0.5, forget plot] table [x=mu, y=nu] {../code/data/2flaveos.dat};
\addplot+ [darkgreen, solid, opacity=0.5, forget plot] table [x=mu, y=nd] {../code/data/2flaveos.dat};
\addplot+ [blue, solid, opacity=0.5, forget plot] table [x=mu, y=ne] {../code/data/2flaveos.dat};
\addplot+ [red, solid] table [x=mu, y=nu] {../code/data/2flavrefeos.dat}; \addlegendentry{$n_u$ (up quarks)};
\addplot+ [darkgreen, solid] table [x=mu, y=nd] {../code/data/2flavrefeos.dat}; \addlegendentry{$n_d$ (down quarks)};
\addplot+ [blue, solid] table [x=mu, y=ne] {../code/data/2flavrefeos.dat}; \addlegendentry{$n_e$ (electrons)};
...
\addplot+ [black, solid, opacity=0.5, forget plot] table [x=P,y=epsilon] {../code/data/2flaveos.dat};
\addplot+ [black, solid] table [x=P,y=epsilon] {../code/data/2flavrefeos.dat};
\end{groupplot}
% Embed zoom-plot in a node
\node [anchor=north east] at (zoomplot) {
	\begin{tikzpicture}[baseline, trim axis left, trim axis right]
	...
	\addplot+ [darkgreen, solid, opacity=0.5, forget plot] table [x=mu, y=nd] {../code/data/2flaveos.dat};
	\addplot+ [blue, solid, opacity=0.5, forget plot] table [x=mu, y=ne] {../code/data/2flaveos.dat};
	\addplot+ [red, solid] table [x=mu, y=nu] {../code/data/2flavrefeos.dat};
	\addplot+ [darkgreen, solid] table [x=mu, y=nd] {../code/data/2flavrefeos.dat};
	\addplot+ [blue, solid] table [x=mu, y=ne] {../code/data/2flavrefeos.dat};
	\end{axis}
	\end{tikzpicture}
};
\end{tikzpicture}
...
\addplot+ [smooth, solid, gray] table [x=R, y=M, meta expr={log10(\thisrow{P}*e0)}] {../code/data/quarkstar2f_B14_27.dat};
...
\addplot+ [smooth, solid, gray] table [x=R, y=M, meta expr={log10(\thisrow{P}*e0)}] {../code/data/quarkstar2f_B14_34.dat};
...
\addplot+ [smooth, solid, gray] table [x=R, y=M, meta expr={log10(\thisrow{P}*e0)}] {../code/data/quarkstar2f_B14_41.dat};
...
\addplot+ [smooth, solid, gray] table [x=R, y=M, meta expr={log10(\thisrow{P}*e0)}] {../code/data/quarkstar2f_B14_48.dat};

\addplot+ [smooth, solid, mesh] table [x=R, y=M, meta expr={log10(\thisrow{P}*e0)}] {../code/data/quarkstar2f_ref_B14_27.dat} node [Bpin, pos=0.45] {$B = (\SI{27}{\mega\electronvolt})^4$};
\addplot+ [smooth, solid, mesh] table [x=R, y=M, meta expr={log10(\thisrow{P}*e0)}] {../code/data/quarkstar2f_ref_B14_34.dat} node [Bpin, pos=0.49] {$B = (\SI{34}{\mega\electronvolt})^4$};
\addplot+ [smooth, solid, mesh] table [x=R, y=M, meta expr={log10(\thisrow{P}*e0)}] {../code/data/quarkstar2f_ref_B14_41.dat} node [Bpin, pos=0.525] {$B = (\SI{41}{\mega\electronvolt})^4$};
\addplot+ [smooth, solid, mesh] table [x=R, y=M, meta expr={log10(\thisrow{P}*e0)}] {../code/data/quarkstar2f_ref_B14_48.dat} node [Bpin, pos=0.555] {$B = (\SI{48}{\mega\electronvolt})^4$};
