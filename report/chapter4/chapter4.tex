...
\caption{\label{fig:lsm:potential}%
	The potential \eqref{eq:lsm:potential} is shaped like a Mexican hat with a continuous region of minima for $\sigma^2 + \vec\pi^2 = 6 m^2 / \lambda$.
	\TODO{include $h \sigma$}
...
\begin{equation}
	...
	\text{\TODO{non-mentioned comment Møte 4}}
	, \qquad
	m_\sigma = \sqrt{2} m
	\qquad \text{and} \qquad
	m_\pi = 0.
	\text{\TODO{$m_\pi^2 \avg{\sigma} = h$ (or similar)}}
\end{equation}
...
\TODO{non-understandable comment, but Goldstone becomes only approximate when $h \neq 0$, and $m_\pi^2 \avg{\sigma} = h$ essentially becomes the approximate Goldstone theorem, only perfectly valid when $h = 0$?}
