...
\section{Color confinement, asymptotic freedom, bag models}
...
Experiments and lattice simulations have shown that color-charged particles like quarks cannot be isolated from each other.
Quarks can be color-charged \textcolor{red}{red}, \textcolor{green}{green} and \textcolor{blue}{blue},
while antiquarks can have be colored with the complementary colors -- or anticolors -- \textcolor{cyan}{antired}, \textcolor{magenta}{antigreen} and \textcolor{yellow}{antiblue} \TODO{}.
Mesons consist of two quarks -- one quark of any color, and one antiquark with the complementary color -- so that the overall meson is color-neutral, or \emph{colorless}, or \emph{white}.
Baryons consist of three quarks -- one red, one green and one blue -- so the baryon is colorless,
while antibaryons consist of three antiquarks -- one of each anticolor -- so the antibaryon is colorless.
The colored quarks are always confined in the interior of hadrons -- i.e. mesons and nucleons.
The color charges are 

\textbf{Asymptotic freedom} is another important property of quantum chromodynamics.
As the energy scale, of interactions between quarks increase, the strength of the interactions \emph{decreases}.
\TODO{ref Gross, Wilczek, Politzer and Nobel Prize}

These two features explain that at high temperature or high density,
quarks are \emph{deconfined} in the sense that they are free of interactions between each other,
but spatially \emph{confined} inside a hadron.

Various \textbf{bag models} have been constructed to describe these properties.
These models picture space to be some medium that is divided into two phases.
The \emph{normal phase} is a void background in which quarks are forbidden to exist.
On top this void, bubbles of \emph{hadron phase} may form, inside which quarks are confined.
Creating one such hadron bubble of volume $V$ out of the normal vacuum phase costs the energy $BV$.

The bag constant $B$ is therefore the difference in energy density between the hadron vacuum phase and the normal vacuum phase.
Accordingly, it creates a negative pressure inside the bag which serves to confine the quarks in the bag.

To determine acceptable values for the bag constant,
we postulate that two-flavor quark matter should be unstable,
and that three-flavor quark matter should be stable (Bodmer-Witten conjecture),
