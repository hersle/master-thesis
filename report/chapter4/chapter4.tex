...
$2$--flavor quark matter should be unstable,
$3$--flavor quark matter should be stable (Bodmer-Witten conjecture),
...
Although we may not be making a great case for ourselves, this is the most common treatment in the literature, and thus the strategy that we will adopt initially.
...
\TODO{will I return to this later?}
...
The numerical solution of the system \eqref{eq:lsm:minsys} is described in \TODO{ref appendix}, and the results are shown in \cref{fig:lsm:2-flavor-eos}.
...
\end{equation}
relative to vacuum and with a bag constant $B$, and the energy density
\begin{equation}
...
More generally, we can allow the pressure to be modified by a \emph{bag constant} $B$:
\begin{equation}
	P(\mu,B) = P(\mu) - B, \quad
	\epsilon(\mu,B) = \epsilon(\mu) + B .
\end{equation}
If vacuum has zero pressure, then subtracting some pressure from the quarks is equivalent to confining them in a ``bag''.
Which values of $B$ should we use?
We have that $P(\mu,B) = P(\mu)-B = 0$ parametrizes bag constants $B = P(\mu)$ by the chemical potential $\mu$.
Since two-flavor quark matter is not observed in nature, we will require that quark matter is unstable with respect to $^{56}\text{Fe}$ with $E/N_B = \SI{930}{\mega\electronvolt}$.
Solving the $2$--flavor part of the stability criterion \eqref{eq:lsm:bag_stability} numerically yields the lower bag pressure bound
\begin{equation}
	B^{1/4} > \SI{27.2}{\mega\electronvolt}.
\end{equation}
