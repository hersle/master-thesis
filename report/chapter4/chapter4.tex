...
To generalize to $SU(3)$, simply replace the three (four) Pauli matrices by the eight (nine) Gell-Mann-matrices.
Can add one more quartic intercation due to \TODO{Hamilton-Caley theorem}.
The three-flavor generalization of the two-flavor quark-meson model Lagrangian \eqref{eq:lsm:lagrangian} is therefore
\begin{equation}
...
\TODO{need to mention $\eta$ meson at some point. but have $m_\eta = 0$ here?}
...
while the two other matrices have the nonzero entries
\begin{equation}
\begin{split}
	\big(m^2_{\sigma\sigma}\big)_{00} &= m^2 + \frac{\lambda_1}{3} \big(4 \sqrt{2} \sigma_x \sigma_y + 7 \sigma_x^2 + 5 \sigma_y^2\big) + \lambda_2 \big(\sigma_x^2 + \sigma_y^2\big) \\
	\big(m^2_{\sigma\sigma}\big)_{11} =
	\big(m^2_{\sigma\sigma}\big)_{22} =
	\big(m^2_{\sigma\sigma}\big)_{33} &= m^2 + \lambda_1 \big(\sigma_x^2 + \sigma_y^2\big)                                              + \frac32 \lambda_2 \sigma_x^2 \\
	\big(m^2_{\sigma\sigma}\big)_{44} =
	\big(m^2_{\sigma\sigma}\big)_{55} =
	\big(m^2_{\sigma\sigma}\big)_{66} =
	\big(m^2_{\sigma\sigma}\big)_{77} &= m^2 + \lambda_1 \big(\sigma_x^2 + \sigma_y^2\big) + \frac{\lambda_2}{2} \big(\sqrt{2} \sigma_x \sigma_y + \sigma_x^2 + 2 \sigma_y^2\big) \\
	\big(m^2_{\sigma\sigma}\big)_{88} &= m^2 - \frac{\lambda_1}{3} \big(4 \sqrt{2} \sigma_x \sigma_y - 5 \sigma_x^2 - 7 \sigma_y^2\big) + \frac{\lambda_2}{2} \big(\sigma_x^2 + 4 \sigma_y^2\big) \\
	\big(m^2_{\sigma\sigma}\big)_{08} =
	\big(m^2_{\sigma\sigma}\big)_{80} &= \frac23 \lambda_1 \big(\sqrt{2} \sigma_x^2 - \sqrt{2} \sigma_y^2 - \sigma_x \sigma_y\big) + \frac{\lambda_2}{\sqrt{2}} \big(\sigma_x^2 - 2 \sigma_y^2\big)
\end{split}%
\label{eq:lsm3f:mass_sigma_sigma}
\end{equation}%
and
\TODO{fix expressions. add $\avg{\sigma}$, or switch to $\bar{\sigma}$ (classical) and $\tilde{\sigma}$ (quantum)}
\begin{equation}
\begin{split}
	\big(m^2_{\pi\pi}\big)_{00} &= m^2 + \lambda_1 \big(\sigma_x^2 + \sigma_y^2\big) + \frac{\lambda_2}{3} \big(\sigma_x^2 + \sigma_y^2\big) \\
	\big(m^2_{\pi\pi}\big)_{11} =
	\big(m^2_{\pi\pi}\big)_{22} =
	\big(m^2_{\pi\pi}\big)_{33} &= m^2 + \lambda_1 \big(\sigma_x^2 + \sigma_y^2\big) + \frac{\lambda_2}{2} \sigma_x^2 \\
	\big(m^2_{\pi\pi}\big)_{44} =
	\big(m^2_{\pi\pi}\big)_{55} =
	\big(m^2_{\pi\pi}\big)_{66} =
	\big(m^2_{\pi\pi}\big)_{77} &= m^2 + \lambda_1 \big(\sigma_x^2 + \sigma_y^2\big) - \frac{\lambda_2}{2} \big(\sqrt{2} \sigma_x \sigma_y - \sigma_x^2 - 2 \sigma_y^2\big) \\
	\big(m^2_{\pi\pi}\big)_{88} &= m^2 + \lambda_1 \big(\sigma_x^2 + \sigma_y^2\big) + \frac{\lambda_2}{6} \big(\sigma_x^2 + 4 \sigma_y^2\big) \\ 
	\big(m^2_{\pi\pi}\big)_{08} =
	\big(m^2_{\pi\pi}\big)_{80} &= \frac{\lambda_2}{6} \big(\sqrt{2} \sigma_x^2 - 2 \sqrt{2} \sigma_y^2\big).
\end{split}%
...
Hence, a fields corresponding to a non-diagonal matrix sector should be rotated to a mass eigenstate basis that diagonalizes the sector.
\begin{itemize}
\item The three diagonal masses $\smash{\big(m^2_{\sigma\sigma}\big)_{11} = \big(m^2_{\sigma\sigma}\big)_{22} = \big(m^2_{\sigma\sigma}\big)_{33}}$ correspond to one degenerate mass $m^2_{a_0}$
      ...
      or alternatively of the two charged $a_0^\pm = (\sigma_1 \pm i \sigma_2) / \sqrt{2}$ mesons and the neutral $a_0^0 = \sigma^3$ meson.
...
\item The \emph{diagonalization} of the non-diagonal matrix sector with elements $\smash{\big(m^2_{\sigma\sigma}\big)_{00}}$, $\smash{\big(m^2_{\sigma\sigma}\big)_{88}}$ and $\smash{\big(m^2_{\sigma\sigma}\big)_{08} = \big(m^2_{\sigma\sigma}\big)_{80}}$ correspond to two masses of the $\sigma$ and $f_0$ mesons,
      achieved by a rotation
      \begin{equation}
      ...
      \label{eq:lsm3f:diagonalization_transformation_sigma}
      \end{equation}
      of the $\sigma_0$ and $\sigma_8$ fields parametrized by a mixing angle $\theta_\sigma$.
\end{itemize}
The situation for the matrix $m^2_{\pi\pi}$ is analogous -- we repeat it verbosely for completeness.
\TODO{need reference for ``alternative'' particles}
\begin{itemize}
\item The three diagonal masses $\smash{\big(m^2_{\pi\pi}\big)_{11} = \big(m^2_{\pi\pi}\big)_{22} = \big(m^2_{\pi\pi}\big)_{33}}$ correspond to one degenerate mass $m^2_\pi$
      of the three $\pi_1$, $\pi_2$ and $\pi_3$ fields,
      or alternatively of the two charged $\pi^\pm = (\pi_1 \pm i \pi_2) / \sqrt{2}$ mesons and the neutral $\pi^0 = \pi^3$ meson.
\item The four diagonal masses $\smash{\big(m^2_{\pi\pi}\big)_{44} = \big(m^2_{\pi\pi}\big)_{55} = \big(m^2_{\pi\pi}\big)_{66}} = \big(m^2_{\pi\pi}\big)_{77}$ correspond to one degenerate mass $m^2_K$
      of the four $\pi_4$, $\pi_5$, $\pi_6$ and $\pi_7$ fields,
      or alternatively of the two charged $K^\pm = (\pi_4 \pm i \pi_5) / \sqrt{2}$ mesons and the two neutral $K^0 = (\pi_6 + i \pi_7) / \sqrt{2}$ and $\bar{K}^0 = (\pi_6 - i \pi_7) / \sqrt{2}$ mesons.
\item The \emph{diagonalization} of the non-diagonal matrix sector with elements $\smash{\big(m^2_{\pi\pi}\big)_{00}}$, $\smash{\big(m^2_{\pi\pi}\big)_{88}}$ and $\smash{\big(m^2_{\pi\pi}\big)_{08} = \big(m^2_{\pi\pi}\big)_{80}}$ correspond to two masses of the $\eta$ and $\eta'$ mesons,
      achieved by a rotation
      \begin{equation}
      ...
      \label{eq:lsm3f:diagonalization_transformation_pi}
      \end{equation}
      of the $\pi_0$ and $\pi_8$ fields parametrized by a mixing angle $\theta_\pi$.
\end{itemize}
To determine the diagonalizing mixing angle $\theta_\pi$,
we use the transformation \eqref{eq:lsm3f:diagonalization_transformation_pi} and the trigonometric identities $\cos^2 \theta_\pi - \sin^2 \theta_\pi = \cos 2 \theta_\pi$ and $2 \sin\theta_\pi \cos\theta_\pi = \sin 2 \theta_\pi$ to expand
\begin{equation}
\begin{split}
	\smash{\smashoperator{\sum_{a,b=\{0,8\}}}} \big(m^2_{\pi\pi}\big)_{ab} \pi_a \pi_b &= \Big\{ \big(m^2_{\pi\pi}\big)_{00} \sin^2 \theta_\pi + \big(m^2_{\pi\pi}\big)_{88} \cos^2 \theta_\pi + \big(m^2_{\pi\pi}\big)_{08} \sin 2 \theta_\pi \Big\} \, \eta^2 \\
	                                                                                   &+ \Big\{ \big(m^2_{\pi\pi}\big)_{00} \cos^2 \theta_\pi + \big(m^2_{\pi\pi}\big)_{88} \sin^2 \theta_\pi - \big(m^2_{\pi\pi}\big)_{08} \sin 2 \theta_\pi \Big\} \, \eta'^2 \\
	                                                                                   &+ \Big\{ 2 \Big[ \big(m^2_{\pi\pi}\big)_{00} - \big(m^2_{\pi\pi}\big)_{88} \Big] \sin 2 \theta_\pi + 2 \big(m^2_{\pi\pi}\big)_{08} \cos 2 \theta_\pi \Big\} \, \eta \eta' ,
\end{split}
\label{eq:lsm3f:diagonalization_sum}
\end{equation}
and then require the coefficient of $\eta \eta'$ to vanish.
...
Let us take a breath and summarize our important findings.
Expressed in terms of the mass matrix elements \eqref{eq:lsm3f:mass_sigma_sigma} and \eqref{eq:lsm3f:mass_pi_pi} and the mixing angles \eqref{eq:lsm3f:mixing_angles},
we have found that the three couplings $m^2$, $\lambda_1$ and $\lambda_2$ generate the eight particle masses
...
\end{subequations}%
\TODO{remark on which masses come from which matrix elements, and particle types}
From any three experimental values of these masses, we can fit the parameters $m^2$, $\lambda_1$ and $\lambda_2$, and then predict values for the five other masses.
\TODO{show table with all used and predicted masses, also for quarks}
...
To fit the six parameters $g$, $m^2$, $\lambda_1$, $\lambda_2$, $h_x$ and $h_8$, we use that the pion and kaon decay constants $f_\pi$ and $f_K$ are related to the ground states through
\cite{ref:lsm3f_details}
\begin{equation}
	f_\pi = \sqrt{\frac23} \avg{\sigma_0} + \frac{1}{\sqrt{3}} \avg{\sigma_8}
	      = \avg{\sigma_x}
	\quad \text{and} \quad
	f_K = \sqrt{\frac23} \avg{\sigma_0} - \frac{1}{\sqrt{12}} \sigma_8
	    = \frac{\avg{\sigma_y}}{\sqrt{2}} + \frac{\avg{\sigma_x}}{2} .
\end{equation}
With experimental values from \cite[]{ref:pdg_review_2021} \TODO{force display as PDG?}, we can then express the vacuum states as
\begin{subequations}
\begin{equation}
	\avg{\sigma_x} = f_\pi
	\quad \text{and} \quad
...
\end{equation}
We determine $m^2$, $\lambda_1$ and $\lambda_2$ by solving the system of equations \eqref{eq:lsm:mass_system_3f} with the meson masses
\cite[]{ref:pdg_review_2021}
\begin{equation}
...
\end{equation}
\end{subequations}
\TODO{find exact locations in PDG, comment on exact numerical choices (e.g. $m_\sigma$)}
This gives
\begin{equation}
\begin{aligned}
...
\begin{tabular}{|c|c|c|c|c|c|c|c|}
	\hline
	$m_{f_0}$ & $m_\sigma$ & $m_{a_0}$ & $m_\kappa$ & $m_\eta$ & $m_{\eta'}$ & $m_\pi$ & $m_K$ \\
	\hline
	\SI{1347}{\mega\electronvolt} & \SI{800}{\mega\electronvolt} & \SI{870}{\mega\electronvolt} & \SI{1141}{\mega\electronvolt} & \SI{636}{\mega\electronvolt} & \SI{138}{\mega\electronvolt} & \SI{138}{\mega\electronvolt} & \SI{496}{\mega\electronvolt} \\
	\hline
\end{tabular}
\fi
\begin{tabular}{|l|r|r|}
	...
	$\lambda_1$ & $-\SI{6.19}{}$                          & \\
	$\lambda_2$ & $-\SI{85.3}{}$                          & \\
	$m^2$       & $-(\SI{492}{\mega\electronvolt})^2$     & \\
...
The \textbf{bold input values} determine the model parameters $g$, $m^2$, $\lambda_1$, $\lambda_2$, $h_x$ and $h_y$,
which in turn predict the non-bold values for remaining particles masses.
}
\end{table}
...
\end{align}
\end{subequations}
Parametrize with common quark chemical potential
\TODO{transform $\mu_u,\mu_d,\mu_s$ to new chemical potentials $\mu_B=3/2(\mu_u+\mu_d),\mu_I=(\mu_u-\mu_d)/2,\mu_S=(\mu_u+\mu_d-2\mu_s)/2$ (verify these!)}
\begin{equation}
	\mu = \mu_u + \mu_d + \mu_e .
\end{equation}
