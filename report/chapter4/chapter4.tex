...
\addlegendentry{two-flavor quark star};
\end{groupplot}
\end{tikzpicture}
\caption{\label{fig:lsm:2-flavor-eos}%
\subref{fig:lsm:2-flavor-eos-parametrization} Solutions to the system of equations \eqref{eq:lsm:minsys} describing charge neutral two-flavor quark matter in $\beta$-equilibrium, parametrized by the chemical potential \eqref{eq:lsm:quark_chemical_potential}.
\subref{fig:lsm:2-flavor-eos-density} Corresponding particle number densities \eqref{eq:lsm:particle_densities}.
\subref{fig:lsm:2-flavor-eos-eos} Corresponding equation of state with no bag constant, found by eliminating the chemical potential from the pressure \eqref{eq:lsm:pressure_bagless} and energy density \eqref{eq:lsm:energy_density_bagless}, compared to the general equation of state for ideal neutron stars in \cref{fig:nstars:eos}.
}
\end{figure}
