...
We determine the mean field $\avg{\sigma}$ by requiring that it minimizes the grand potential according to
\begin{equation}
	\pdv{\Omega}{\avg{\sigma}} = 0.
\label{eq:lsm:potential_minimize}
\end{equation}
To get a feeling for the general shape of the potential, we visualize the special case with $\mu_e = 0$ and $\mu_u = \mu_d = \mu$ in \cref{fig:lsm:grand-potential-nonneutral}.
...
\item For $\mu < \SI{300}{\mega\electronvolt} = m_q(f_\pi)$, no square roots have real parts, so we are in vacuum with minimum at $\avg{\sigma} = f_\pi = \SI{93}{\mega\electronvolt}$, as we required in equation \eqref{eq:lsm:potential_vacuum_minimum}.
      The property that a \emph{range} of chemical potentials characterizes the vacuum is called the \emph{Silver Blaze property}. \TODO{some etymology?}
...
Using that up and down quarks have respective charges $+(2/3)e$ and $-(1/3)e$ and counting color degeneracy, we should now solve the \emph{system} of equations
\begin{subequations}
\begin{align}
	0 &= \pdv{\Omega}{\avg{\sigma}} , \label{eq:lsm:minsys_min} \\
	0 &= \frac23 N_c \left( \mu_u^2 - m_q^2 \right)^{3/2} - \frac13 N_c \left( \mu_d^2 - m_q^2 \right)^{3/2} - \left( \mu_e^2 - m_e^2 \right)^{3/2} , \label{eq:lsm:minsys_charge} \\
	\mu_d &= \mu_u + \mu_e . \label{eq:lsm:minsys_equilibrium}
\end{align}%
\label{eq:lsm:minsys}%
\end{subequations}%
In this case $\Omega(\avg{\sigma},\mu_u,\mu_d,\mu_e)$ is a function of \emph{four} arguments,
constrained by a system of \emph{three} equations,
...
The numerical solution of the system \eqref{eq:lsm:minsys} is described in \TODO{ref appendix}.
This yields the parametrizations $\avg{\sigma}(\mu)$, $\mu_u(\mu)$, $\mu_d(\mu)$ and $\mu_e(\mu)$ shown in \cref{fig:lsm:2-flavor-eos-parametrization}.
From these parametrizations, we compute the particle number densities \TODO{ref some general formula instead, from intro?}
\begin{equation}
	n_u = \frac{N_c}{3\pi^2} ( \mu_u^2-m_q   )^{3/2}, \quad
	n_d = \frac{N_c}{3\pi^2} ( \mu_d^2-m_q^2 )^{3/2}  \quad \text{and} \quad
	n_e = \frac{1}{3\pi^2}   ( \mu_e^2-m_e^2 )^{3/2}
\end{equation}
shown in \cref{fig:lsm:2-flavor-eos-density}.
Finally, we compute the pressure $P(\mu) = -[\Omega(\mu) - \Omega(\mu<\SI{300}{\mega\electronvolt})]$ relative to vacuum and the energy density $\epsilon(\mu) = -P(\mu) + \sum_i \mu_i(\mu) \, n_i(\mu)$,
then at last eliminate the free parameter $\mu$ to obtain the equation of state $\epsilon(P)$ in \cref{fig:lsm:2-flavor-eos-eos}.

\TODO{bag constant}

With the equation of state, we now solve the Tolman-Oppenheimer-Volkoff equations \TODO{new ref?} with multiple bag pressures above the lower bound \TODO{ref}.
This yields the mass-radius curve parametrized by central pressure in \cref{fig:lsm:2-flavor-mass-radius}.
The $2$--flavor quark stars have maximum masses in the range $1.7 M_\odot < M_\text{max} < 1.8 M_\odot$, depending on the precise bag constant.
...
\nextgroupplot[
	...
	xmin=0, xmax=600, ytick distance=100,
	ymax=600, 
	%xmin=600, xmax=1400, 
	%ymin=-10, ymax=100, 
...
\nextgroupplot[
	...
	xmin=0, xmax=600, ymax=3, ytick distance=1,
	%xmin=600, xmax=1400,
...
\subref{fig:lsm:2-flavor-eos-parametrization} Potential-minimizing mean field parametrization,
...
\end{axis}
\end{tikzpicture}
\caption{\label{fig:lsm:2-flavor-mass-radius}Mass-radius solutions of the Tolman-Oppenheimer-Volkoff equations \TODO{ref} with the equation of state \TODO{ref} for $2$--flavor quark star with multiple bag constants $B$.}
\end{figure}
