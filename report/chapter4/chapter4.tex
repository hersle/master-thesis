...
\nextgroupplot[
	...
	xmin=0, xmax=600, ymax=500, ytick distance=100, minor x tick num=4,
	%ymax=600, 
...
\nextgroupplot[
	...
	xmin=0, xmax=600, minor x tick num=4,
...
\begin{equation}
\begin{split}
	\lagr &= \sum_{c=1}^{N_c} \bar{q}_c [ i \slashed\partial - g T_a (\sigma_a + i \gamma_5 \pi_a) ] q + \trace[(\partial_\mu \phi)^\dagger (\partial_\mu \phi)] \\
	      &- m^2 \trace[\phi^\dagger \phi] - \lambda_1 [\trace(\phi^\dagger \phi)]^2 - \lambda_2 \trace[(\phi^\dagger \phi)^2] + c \tdet(\phi^\dagger \phi) + \trace[H(\phi+\phi^\dagger)]
\end{split}
\end{equation}
Field
\begin{equation}
...
\end{NiceMatrixBlock}
\end{equation}
Set $c=h_1=h_2=h_3=h_4=h_5=h_6=h_7=0$,
leaving 6 unknown parameters $g$, $m^2$, $\lambda_1$, $\lambda_2$, $h_0$, $h_8$.
Field transformation
(arises from wanting $\trace \phi^4$ to be decoupled into separate terms. look for an orthogonal transformation that accomplishes this (i.e. diagonalizes $\lagr$?) (3 equations: 4 matrix elements except 1 due to symmetry). this gives three coefficients in front of terms $\sigma_0^1 \sigma_8^3$, $\sigma_0^2 \sigma_8^2$ and $\sigma_0^3 \sigma_8^1$ that we require to be zero).
\TODO{use bNiceMatrix?}
\begin{equation}
...
\end{equation}
Calculate $d_{abc}$ explicitly from equation above to find
\TODO{just do the traces on a machine}
\begin{equation}
	\trace[(\avg{\phi}^\dagger \avg{\phi})^2] = \frac18 \sigma_x^4 + \frac14 \sigma_y^4
\end{equation}
The expansion of the Lagrangian is therefore
\TODO{$\lambda/4!$ for consistency}
\begin{equation}
	\lagr = \sum_{c=1}^{N_c} \bar{q}_c [ i \slashed\partial - g T_a (\avg{\sigma_a} ] q + \text{\TODO{no pdv term?}} - \pot 
\end{equation}
with the potential
\begin{equation}
	\pot = \frac12 m^2 [\avg{\sigma_x}^2 + \avg{\sigma_y}^2] + \frac{\lambda_1}{4} [\avg{\sigma_x}^2 + \avg{\sigma_y}^2]^2 + \lambda_2 [\frac18 \avg{\sigma_x}^4 + \frac14 \avg{\sigma_y}^4] - h_x \avg{\sigma_x} - h_y \avg{\sigma_y}
\end{equation}
The potential $\pot$ has a minimum located at
\begin{equation}
	\avg{\sigma_0} \neq 0, \quad
	\avg{\sigma_8} \neq 0, \quad
	\avg{\sigma_a} = 0 \text{ for $a \neq 0$ and $a \neq 8$}, \quad
	\avg{\pi_a} = 0 \text{ for all $a=0, \ldots, 8$}.
\end{equation}
Thus, shifting
\begin{equation}
	\sigma \rightarrow \avg{\sigma} + \sigma \quad \text{and} \quad \pi \rightarrow \avg{\pi} + \pi,
\end{equation}
the potential can be expanded to
\begin{equation}
	\pot = \pot(\avg{\sigma}, \avg{\pi}) + \frac12 \big(m^2_{\sigma\sigma}\big)_{ab} \sigma_a \sigma_b + \frac12 \big(m^2_{\sigma\pi}\big)_{ab} \sigma_a \pi_b + \frac12 \big(m^2_{\pi\pi}\big)_{ab} \pi_a \pi_b
\end{equation}
with the tree-level potential
\begin{equation}
	\pot(\avg{\sigma}, \avg{\pi}) = \frac{1}{2} m^2 (\avg{\sigma_x}^2 + \avg{\sigma_y}^2) + \frac{\lambda_1}{4} (\avg{\sigma_x}^2 + \avg{\sigma_y}^2)^2 + \frac{\lambda_2 }{8} (\avg{\sigma_x}^4 + 2 \avg{\sigma_y}^4) - h_x \avg{\sigma_x} - h_y \avg{\sigma_y}
\end{equation}
and the three mass matrices
\begin{subequations}
\begin{align}
	\big(m^2_{\sigma\sigma}\big)_{ab} &= \pdv[2]{\pot}{\sigma_a \sigma_b}_{(\sigma,\pi)=(\avg{\sigma},\avg{\pi})}, \\
	\big(m^2_{\sigma\pi}\big)_{ab}    &= \pdv[2]{\pot}{\sigma_a \pi_b}_{(\sigma,\pi)=(\avg{\sigma},\avg{\pi})}, \\
	\big(m^2_{\pi\pi}\big)_{ab}       &= \pdv[2]{\pot}{\pi_a \pi_b}_{(\sigma,\pi)=(\avg{\sigma},\avg{\pi})}.
\end{align}%
\end{subequations}%
The mixed mass matrix $m^2_{\sigma\pi} = 0$ vanishes because $\avg{\pi} = 0$.
Diagonalize the remaining symmetric mass matrices with a unitary transformation $\sigma \rightarrow \bar{\sigma} = U \sigma$:
\begin{equation}
	\sigma^T m^2_{\sigma\sigma} \sigma = \sigma^T U^T U m^2_{\sigma\sigma} U^T U \sigma = (U \sigma)^T U^T m^2_{\sigma\sigma} U (U \sigma) = \bar{\sigma}^T m^2_\sigma \bar\sigma.
\end{equation}
Then the potential is
\begin{equation}
	\pot = \pot(\avg{\sigma}, \avg{\pi}) + \frac12 \big(m^2_{\sigma}\big)_{aa} \bar{\sigma}_a^2 + \frac12 \big(m^2_{\pi\pi}\big)_{ab} \bar{\pi}_a^2.
\end{equation}
Include mesons $\sigma (\times 1)$, $\pi (\times 3)$ and $K (\times 4)$ with eigenvalue masses \TODO{justify choice of eigenvalues}
\begin{subequations}
\begin{align}
	m^2_\sigma &= m^2 + 2 \lambda_1 (\sigma_x^2 + \sigma_y^2) + \frac{3}{4} \lambda_2 (\sigma_x^2 + 2 \lambda_2 \sigma_y^2) \nonumber \\
	           &- \frac{1}{4} \sqrt{16 \lambda_1^2 (\sigma_x^2 + \sigma_y^2)^2 + 9 \lambda_2^2 (\sigma_x^4 - 4 \sigma_x^2 \sigma_y^2 + 4 \sigma_y^4) + 24 \lambda_1 \lambda_2 (\sigma_x^4 - 3 \sigma_x^2 \sigma_y^2 + 2 \sigma_y^4)}, \\
	%m^2_\pi    &= m^2 + \lambda_1 (\sigma_0^2 + \sigma_8^2) + \frac{\lambda_2}{6} (2 \sqrt{2} \sigma_0 \sigma_8 + 2 \sigma_0^2 + \sigma_8^2), \\
	m^2_\pi    &= m^2 + \lambda_1 (\sigma_x^2 + \sigma_y^2) + \frac12 \lambda_2 \sigma_x^2, \\
	%m^2_K      &= m^2 + \lambda_1 (\sigma_0^2 + \sigma_8^2) - \frac{\lambda_2}{6} (\sqrt{2} \sigma_0 \sigma_8 - 2 \sigma_0^2 - 7 \sigma_8^2). \\
	m^2_K      &= m^2 + \lambda_1 (\sigma_x^2 + \sigma_y^2) + \frac12 \lambda_2 (\sigma_x^2 + 2 \sigma_y^2 - \sqrt{2} \sigma_x \sigma_y).
\end{align}
\end{subequations}
Use these equations to determine $m^2$, $\lambda_1$ and $\lambda_2$ from measurements of $m_\sigma$, $m_\pi$ and $m_K$.

Determine symmetry breaking parameters $h_x$ and $h_y$ from
\begin{equation}
	\pdv{\pot}{\sigma_x}_{0} = \pdv{\pot}{\sigma_y}_{0} = 0.
\end{equation}
This gives (eliminate $\lambda_1$ and $\lambda_2$ using equations for $m^2_\pi$ and $m^2_K$)
\begin{subequations}
\begin{align}
	%h_x &= \frac12 (2 \lambda_1 + \lambda_2) \sigma_x^3 + \lambda_1 \sigma_x \sigma_y^2 + m^2 \sigma_x = \sigma_x m_\pi^2, \\
	h_x &= \sigma_x [m^2 + \lambda_1 (\sigma_x^2 + \sigma_y^2) + \frac12 \lambda_2 \sigma_y^2] = \sigma_x m_\pi^2, \\
	%h_y &= (\lambda_1 + \lambda_2) \sigma_y^3 + (\lambda_1 \sigma_x^2 + m^2) \sigma_y..
	h_y &= \sigma_y [m^2 + \lambda_1 (\sigma_x^2 + \sigma_y^2) + \lambda_2 \sigma_y^2] = \sqrt{2} f_K m_K^2 - \frac{1}{\sqrt{2}} f_\pi m_\pi^2.
\end{align}
\end{subequations}

Finally, quarks have masses
\begin{equation}
	m_u = g (T_a)_{11} \avg{\sigma_a} = \frac{g \sigma_x}{2}, \quad
	m_d = g (T_a)_{22} \avg{\sigma_a} = \frac{g \sigma_x}{2}  \quad \text{and} \quad
	m_s = g (T_a)_{22} \avg{\sigma_a} = \frac{g \sigma_y}{\sqrt{2}}.
\end{equation}
Determine $g$ from
\begin{equation}
	g = \frac{2 m_{u/d}^0}{\sigma_x} \quad \text{with} \quad m_{u/d}^0 = \SI{300}{\mega\electronvolt} \quad \text{and} \quad \sigma_x = f_\pi = \SI{93}{\mega\electronvolt}.
\end{equation}

Using the numerical vacuum values
\begin{subequations}
\begin{align}
	f_\pi    &= \SI{93}{\mega\electronvolt},  & f_K   &= \SI{113}{\mega\electronvolt}, \\
	m_\sigma &= \SI{800}{\mega\electronvolt}, & m_\pi &= \SI{138}{\mega\electronvolt}, & m_K &= \SI{496}{\mega\electronvolt}.
\end{align}
\end{subequations}
we obtain the parameters
\begin{subequations}
\begin{align}
	m^2 &= -(\SI{491.7}{\mega\electronvolt})^2, & \lambda_1 &= -\SI{6.19}{}, & \lambda_2 &= \SI{85.3}{}, \\
	h_x &= (\SI{121.0}{\mega\electronvolt})^3, & h_y &= (\SI{336.4}{\mega\electronvolt})^3, \\
	g &= \SI{6.5}{}.
\end{align}
\end{subequations}
...
\begin{split}
	...
	-& \frac{N_c}{24 \pi^2} \sum_{f=\{u,d\}} \left[ \left( 2 \mu_f^2 - 5 m_f^2 \right) \mu_f \sqrt{\mu_f^2 - m_f^2} + 3 m_f^4 \asinh \left( \sqrt{\frac{\mu_{\smash{f}}^2}{m_q^2}-1} \right) \right] \\
	-& \frac{  1}{24 \pi^2} \left[ \left( 2 \mu_e^2 - 5 m_e^2 \right) \mu_e \sqrt{\mu_e^2 - m_e^2} + 3 m_e^4 \asinh \left( \sqrt{\frac{\mu_e^2}{m_e^2}-1} \right) \right] \\
	+& N_c \sum_f \frac{m_f^4}{16 \pi^2} \left[ \frac{1}{\epsilon} + \frac{3}{2} + \log\left(\frac{{\Lambda}^2}{m_f^2}\right) \right].
\end{split}
\end{equation}
Has divergent term
\begin{equation}
	\frac{N_c g^4}{16 \pi^2} \frac18 (\sigma_x^4 + 2 \sigma_y^4)
\end{equation}
Remove by renormalizing
\begin{equation}
	\lambda_2 \rightarrow \lambda_2^P + \delta\lambda_2 \qquad \text{with} \qquad \delta\lambda_2 = -\frac{N_c g^4}{16 \pi^2} .
\end{equation}
How to determine $\Lambda$?
Minimize potential in both directions in vacuum to find $\Lambda_x$ and $\Lambda_y$:
\begin{equation}
	\pdv{\Omega}{\avg{\sigma_x}} = \pdv{\Omega}{\avg{\sigma_y}} = 0.
\end{equation}
Gives $\Lambda_x = g \sigma_x / 2 \sqrt{e} = \SI{182.0}{\mega\electronvolt}$ and $\Lambda_y = g \sigma_y / \sqrt{2 e} = \SI{260.2}{\mega\electronvolt}$.
But $\Lambda$ can only have one value, so choose the average ($u,d,s$ weigh equally)
\begin{equation}
	\Lambda = \frac{2 \Lambda_x + \Lambda_y}{3} = \SI{208.0}{\mega\electronvolt}.
\end{equation}

\subsection{Equation of state etc}
...
\end{align}
\end{subequations}
Parametrize with quark chemical potential
\begin{equation}
	\mu = \mu_u + \mu_d + \mu_e .
\end{equation}
...
\nextgroupplot[
	xlabel={$P        \, / \, (\si{\giga\electronvolt\per\femto\meter\cubed})$},
	ylabel={$\epsilon \, / \, (\si{\giga\electronvolt\per\femto\meter\cubed})$},
	xmin=0, xmax=1.0, ymin=0, ymax=3, xtick distance=0.1, ytick distance=0.5, minor y tick num=4,
	%title={\subcaption{\label{fig:lsm:2-flavor-eos-eos}Equation of state}},
	legend cell align=left, legend pos=north west,
];
%\addplot+ [black, densely dashed, semithick, opacity=0.7, forget plot] table [x=P,y=epsilon] {../code/data/2flavfreeeos.dat}; %\addlegendentry{without interactions};
...
\addplot+ [solid, gray, opacity=0.5] table [x=R, y=M, meta expr={log10(\thisrow{P}*e0)}] {../code/data/quarkstar2f_B14_27.dat}; \addlegendentry{two-flavor \TODO{indicate stable/unstable}};
...
\addplot+ [solid, mesh] table [x=R, y=M, meta expr={log10(\thisrow{P}*e0)}] {../code/data/quarkstar3f_B14_27.dat}; \addlegendentry{three-flavor \TODO{indicate stable/unstable}};
%\addplot+ [solid,          mesh, x filter/.expression={\thisrow{P} < 0.00139 ? x : nan}] table [x=R, y=M, meta expr={log10(\thisrow{P}*e0)}] {../code/data/quarkstar2f_B14_27.dat} node [Bpin, pos=0.82] {$B = (\SI{27}{\mega\electronvolt})^4$};
%\addplot+ [densely dashed, mesh, x filter/.expression={\thisrow{P} > 0.00138 ? x : nan}] table [x=R, y=M, meta expr={log10(\thisrow{P}*e0)}] {../code/data/quarkstar2f_B14_27.dat};

\addplot+ [solid, mesh] table [x=R, y=M, meta expr={log10(\thisrow{P}*e0)}] {../code/data/quarkstar3f_B14_34.dat};
%\addplot+ [solid,          mesh, x filter/.expression={\thisrow{P} < 0.0015 ? x : nan}] table [x=R, y=M, meta expr={log10(\thisrow{P}*e0)}] {../code/data/quarkstar2f_B14_34.dat} node [Bpin, pos=0.77] {$B = (\SI{34}{\mega\electronvolt})^4$};
%\addplot+ [densely dashed, mesh, x filter/.expression={\thisrow{P} > 0.0014 ? x : nan}] table [x=R, y=M, meta expr={log10(\thisrow{P}*e0)}] {../code/data/quarkstar2f_B14_34.dat};

\addplot+ [solid, mesh] table [x=R, y=M, meta expr={log10(\thisrow{P}*e0)}] {../code/data/quarkstar3f_B14_41.dat};
%\addplot+ [solid,          mesh, x filter/.expression={\thisrow{P} < 0.0013 ? x : nan}] table [x=R, y=M, meta expr={log10(\thisrow{P}*e0)}] {../code/data/quarkstar2f_B14_41.dat} node [Bpin, pos=0.70] {$B = (\SI{41}{\mega\electronvolt})^4$};
%\addplot+ [densely dashed, mesh, x filter/.expression={\thisrow{P} > 0.0012 ? x : nan}] table [x=R, y=M, meta expr={log10(\thisrow{P}*e0)}] {../code/data/quarkstar2f_B14_41.dat};

\addplot+ [solid, mesh] table [x=R, y=M, meta expr={log10(\thisrow{P}*e0)}] {../code/data/quarkstar3f_B14_48.dat};
%\addplot+ [solid,          mesh, x filter/.expression={\thisrow{P} < 0.0016 ? x : nan}] table [x=R, y=M, meta expr={log10(\thisrow{P}*e0)}] {../code/data/quarkstar2f_B14_48.dat} node [Bpin, pos=0.47] {$B = (\SI{48}{\mega\electronvolt})^4$};
%\addplot+ [densely dashed, mesh, x filter/.expression={\thisrow{P} > 0.0015 ? x : nan}] table [x=R, y=M, meta expr={log10(\thisrow{P}*e0)}] {../code/data/quarkstar2f_B14_48.dat};
