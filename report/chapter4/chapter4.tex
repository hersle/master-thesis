...
\section{Charge neutrality and chemical equilibrium in stars}
...
The stars consisted of only one charge neutral particle, and we simply eliminated its chemical potential to find an equation of state.
Now we will consider stellar models with a greater variety of particles and non-zero charge.

How do we now find the equation of state, when there is one chemical potential for each kind of particle?
The answer is to impose additional physical requirements of charge neutrality and chemical equilibrium.
...
\subsection{Global charge neutrality}

We can make a very simple classical argument for why there can be no \emph{global} charge in stars by comparing Newton's law of gravity and Coulomb's law.
Consider a test particle of mass $m$ and charge $q$ at the surface $R$ from the center of a star with total mass $M$ and charge $Q$.
...
Any charged particle has about one elementary charge $q \approx \pm e$.
...
This corresponds to practically zero charge per particle in the star.

This means that particle are expelled from the star while $Z > 10^{-37} N$ until $Z$ has fallen to \emph{at least} $Z < 10^{-37} N$.
We conclude that for all practical purposes, stars are \emph{globally} charge neutral.
...
\subsection{Local charge neutrality}

The argument in the previous section shows that a star can have no \emph{total} charge, but it places absolutely no limitations on its \emph{distribution}.
...
To make use of charge neutrality in our approach, we will promote charge neutrality from a \emph{global} constraint to a \emph{local} one.
...
\TODO{write about this: find some important reactions, demand them to be in equilibrium and relate chemical potentials}

\section{Linear sigma model}

\TODO{section on solving TOV with more particles using charge neutrality and chemical equilibrium}
...
\begin{axis}[
	...
	\addplot3 [surf, thin, domain=0:3.0, domain y=0:2*pi, samples=30, samples y=40, z buffer=sort] ({x*cos(deg(y))},{x*sin(deg(y))},{-1/2*((x*cos(deg(y)))^2+(x*sin(deg(y)))^2) + 1/24*((x*cos(deg(y)))^2+(x*sin(deg(y)))^2)^2 + 3/2});
...
\end{equation}
\TODO{should I have $h \sigma$, too?}
with the potential
\begin{equation}
