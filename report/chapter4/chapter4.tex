...
To investigate the composition of the stars, we plot the radial density profile of the present particles in \cref{fig:lsm3f:3-flavor-radial-density}.
The two figures show many interesting features:
\begin{itemize}
...
\item Only stars with central pressure near or beyond that corresponding to the maximum mass have a nonzero strange quark population.
      \Cref{fig:lsm3f:3-flavor-radial-density} shows that stars with central pressure exceeding $P_c \gtrsim 10^{34.41} \si{\pascal} = \SI{0.16}{\giga\electronvolt\per\femto\meter\cubed}$ have a core partly composed of strange quarks.
...
\item Masses and radii of stars composed of only up and down quarks differ greatly depending on whether they are modeled with the three-flavor or two-flavor model.
      These stars correspond to those below $P_c \gtrsim 10^{34.41} \si{\pascal} = \SI{0.16}{\giga\electronvolt\per\femto\meter\cubed}$.
      ...
      Apart from simplicity, the reason we study models including up, down and strange quarks, but not charm, bottom and top quarks is that the three latter quarks would only be present at stars with even greater density,
      located beyond the maximum mass and thus being unstable and unphysical stars.
      Nevertheless, given the difference in results from the strange quark's inclusion in the model, but physical absence, it would be interesting to see how this effect would play out with the inclusion of charm, bottom and top quarks.
\item Unlike the corresponding radii, the maximum masses are resistant against changes in the bag constant within the bag window we have considered.
      With the picture that the bag constant controls the size of the hadronic bags that confine the quarks,
      it is as if increasing the bag size only ``inflates'' the bag and make them occupy more space without affecting the total mass.
      For stars with lower pressure, however, the bag constant significantly modifies the equation of state and thus both masses and radii.
\item \TODO{add to this list!}
\item \TODO{interpret non-quark terms in pressure as effective mass-dependent bag pressure? then $B$ normalizes its scale?}
...
\begin{axis}[
	...
	xmin=5, xmax=18, ymin=0.0, ymax=1.8, xtick distance=1, ytick distance=0.1, minor tick num=9, grid=major,
	point meta=explicit, point meta min=32, point meta max=39,
	colorbar horizontal, colormap name=plasmarev, colorbar style={xlabel=$\log_{10} (P_c \, / \, \si{\pascal})$, xtick distance=1, minor x tick num=9, at={(0.5,1.03)}, anchor=south, xticklabel pos=upper},
	declare function={
		e0 = 4.266500881855304e+37;
