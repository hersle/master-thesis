...
In \cref{sec:weak_field_limit} we will see that Buchdal's mass-radius bound is a relativistic result and that no such bound arises from Newtonian gravity.
...
In \cref{sec:incompressible_star}, we argued that a star with constant energy density $\epsilon_0$ is the one that can withstand the most extreme pressure.
...
\begin{equation}
	...
	\label{eq:weak_field_limit:central_pressure_newtonian}
\end{equation}
...
Finally, for a star with non-increasing energy density $\epsilon(r)$ away from the center $r=0$ -- which we deemed to be the only physical type of stars in \cref{sec:incompressible_star} -- we can pull the minimum density $\epsilon(r)$ outside the integral \eqref{eq:einstein_to_tov:m_integral} to get $m(r) \geq \frac{4}{3} \pi r^3 \epsilon(r)$, so
\begin{equation*}
...
Finally, let us examine whether Newtonian gravity enforces any limitations on masses and radii of stars, like the Buchdal bound \eqref{eq:incompressible_star:buchdal}.
Solving the Newtonian equation \eqref{eq:weak_field_limit:newtonian_pressure_gradient} for a star of constant mass density $\epsilon(r) = \epsilon_0$ like we solved the TOV equation \eqref{eq:tov} in \cref{sec:incompressible_star}, we get the pressures
\begin{equation}
	p(r) = \frac{\epsilon_0}{2} \frac{G M}{R} \left( 1 + \frac{r}{R} \right) \left( 1 - \frac{r}{R} \right)
	\quad \text{and} \quad
	p(0) = \frac{\epsilon_0}{2} \frac{GM}{R} .
\end{equation}
This is exactly the Taylor expansion of the relativistic pressure \eqref{eq:incompressible_star:pressure}, as we would expect -- the solution of a Taylor expanded differential equation should be the same as the Taylor expansion of the solution to a differential equation.
In this case, the pressure is well-behaved for all $r$.
This shows that the Buchdal mass-radius bound is a purely relativistic result, and that no such limitation arises in Newtonian gravity!

(TODO: merge this Newtonian solution into the arguments made above of $GM/R \ll 1$, etc. by doing this solution first, then show the reduction to the Newtonian equation)
