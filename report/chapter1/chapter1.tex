...
For example, sound waves propagagate with velocity $v = \sqrt{\difft{p}{\rho}} = c \sqrt{\difft{T_{ij}}{T_{00}}}$.
We must then have $\difft{T_{ij}}{T_{00}} \ll 1$ and, by integration,
\begin{equation}
	T_{ij} \ll T_{00} .
...
The criterion of slow movement \cref{eq:weak_field_limit:small_velocity} implies that a particle with velocity $\difft{x^\mu}{\tau}$ has spatial components $\difft{x^i}{t} \ll c$ and must be dominated by the temporal component $c \difft{t}{\tau} \approx c$.
...
\end{equation}
In particular, the static metric implies that $\Gamma^0_{00} = 0$, so $\diff[2]{t}{\tau} = 0$ (TODO: use derivative package with text-mode option) by the approximated geodesic equation \eqref{eq:weak_field_limit:geodesic_approximated}.
As we have neglected $\difft{x^i}{\tau}$, we can now regard $x(\tau) = x(\tau(t))$ as a function of $t$ and apply the chain rule and the product rule to get $\diff[2]{x}{\tau} = \diff[2]{x}{t} (\diff{t}{\tau})^2 + \diff{x}{t} \diff[2]{t}{\tau} = \diff[2]{x}{t} (\diff{t}{\tau})^2$.
Upon insertion into the \cref{eq:weak_field_limit:geodesic_approximated}, we can cancel the common factor $(\diff{t}{\tau})^2$ and write the geodesic equation as
\begin{equation*}
...
\end{equation}
Inserting our results from above, we find the lowest nonzero order Ricci tensor element
\begin{equation}
	R_{00} = \frac{4 \pi G \rho}{c^2} .
	\label{eq:weak_field_limit:ricci_from_energy_momentum}
\end{equation}

Instead of calculating the Ricci tensor from matter, we can attack the problem from the geometric point of view and find it from the Riemann curvature tensor
\begin{equation}
...
Inserting $h_{00} = -2V/c^2$ from \cref{eq:weak_field_limit:metric_with_potential} and comparing the result to \cref{eq:weak_field_limit:ricci_from_energy_momentum}, we have indeed found the Newtonian response \eqref{eq:interpretation_m:poisson}!
...
Outside a star, spacetime is described by the Schwarzschild metric with the element
\begin{equation}
