...
Given all three equations \eqref{eq:einstein_to_tov:m_rho}, \eqref{eq:tov} and \eqref{eq:tov:eos} and the central pressure $P(0)$, we can integrate to find the pressure everywhere inside the star.
...
Together with the mass gradient \eqref{eq:einstein_to_tov:m_rho}, one of the equations involving $\alpha(r)$ such as \cref{eq:einstein_to_tov:dadr1} and an equation of state \eqref{eq:tov:eos}, it is best viewed as the system of equations
\begin{subequations}
\label{eq:tov:tovsys}
\begin{align}
...
If needed, one can also integrate \eqref{eq:tov:tovsys_alpha} subject to $\alpha(R) = \log \left( 1 - 2 G M / r c^2 \right)$ to determine $\alpha(r)$ and hence the full form of the metric \eqref{eq:tov:metric} after calculating $\beta(r)$ from $m(r)$ through equation \eqref{eq:einstein_to_tov:def_m}.
