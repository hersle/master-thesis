...
def relativistic_pressure():
    p(r) = function("p")(r)
    diffeq = diff(p(r),r) == -G*m(r)*e0/(r^2*c^2) * (1+p(r)/e0) * (1+4*pi*r^3*p(r)/(m(r)*c^2)) / (1-2*G*m(r)/(r*c^2))
    ...
    p(r) = p(r).substitute(e0 = M*c^2 / (4/3*pi*R^3))
    p(r) = p(r).simplify_full().factor()
    return p(r)

def newtonian_pressure():
    p(r) = function("p")(r)
    diffeq = diff(p(r),r) == -G*m(r)*e0/(r^2*c^2)
    p(r) = desolve(diffeq, dvar=p(r), ivar=r, ics=ics)
    p(r) = p(r).substitute(e0^2 == e0 * M*c^2/(4/3*pi*R^3))
    p(r) = p(r).factor()
    return p(r)

def plot_pressure(p, M, R, c=1, **kwds):
    f(x) = (p(x*R)/p(0)).substitute(M=M).substitute(R=R).substitute(c=c).real()
...
print(bool((p2(r).substitute(G=R*c^2/M*x).simplify_full().taylor(x, 0, 1).substitute(x=G*M/(R*c^2)) - p1(r)).substitute(e0=M*c^2/(4*pi*R^3/3)).simplify_full() == 0))
